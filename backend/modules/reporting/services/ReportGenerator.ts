...
建议数量: ${summary.recommendations}
  ...
  async generateChart(config: ChartConfig): Promise<string> {
    ...
    const padding = 40;
    const values = config.data.map(value => {
    ...
    const buildBars = () => {
      ...
      return values
        .map((value, index) => {
          ...
          return `<rect x="${x}" y="${y}" width="${w}" height="${barHeight}" fill="#4f46e5" />`;
        })
        .join('');
    };
    ...
    const buildAreaPath = () => {
      ...
      return `${buildLinePath()} L${points[points.length - 1].x},${baseY} L${points[0].x},${baseY} Z`;
    };

    const buildScatter = () =>
      points
        .map(point => `<circle cx="${point.x}" cy="${point.y}" r="4" fill="#4f46e5" />`)
        .join('');
    ...
    const buildPie = () => {
      ...
      return values
        .map((value, index) => {
          ...
          const colorHue = (index * 360) / Math.max(values.length, 1);
          const path = `M${centerX},${centerY} L${x1},${y1} A${radius},${radius} 0 ${largeArc} 1 ${x2},${y2} Z`;
          startAngle = endAngle;
          return `<path d="${path}" fill="hsl(${colorHue},70%,60%)" />`;
        })
        .join('');
    };
    ...
    if (config.type === 'bar') {
      ...
      chartBody = `<path d="${buildLinePath()}" fill="none" stroke="#4f46e5" stroke-width="2" />`;
    } else if (config.type === 'area') {
      chartBody = `<path d="${buildAreaPath()}" fill="rgba(79, 70, 229, 0.2)" stroke="#4f46e5" stroke-width="2" />`;
    } else if (config.type === 'scatter') {
      chartBody = buildScatter();
    ...
    return `
      <div class="chart" style="width: ${width}px; height: ${height}px;">
        <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-label="${config.title}">
          <rect width="100%" height="100%" fill="white" />
          <text x="${padding}" y="${padding - 12}" font-size="16" fill="#111827">${config.title}</text>
          ${chartBody}
        </svg>
      </div>
    `;
