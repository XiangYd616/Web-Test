 ...
 * 报告控制器
 * 职责: 处理报告相关的业务逻辑
 * 从 system/routes/reports.ts 中提取
 */
...
import type { ReportTemplate as ServiceReportTemplate } from '../services/AutomatedReportingService';
...
const validateShareAccess = (
  ...
  if (share.is_active === false) return { allowed: false, message: '分享已失效' };
  ...
  if (!permissions.includes(requiredPermission)) return { allowed: false, message: '无访问权限' };
...
const insertTemplateVersion = async (
  ...
  const versionResult = await query(
    `SELECT COALESCE(MAX(version), 0) + 1 AS next_version FROM data_record_versions WHERE record_id = $1 AND type = $2`,
    [templateId, 'report_template']
  );
  const version = Number(versionResult.rows[0]?.next_version || 1);
  await query(
    `INSERT INTO data_record_versions (record_id, type, workspace_id, data, metadata, version, action, created_by) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
    [
      templateId,
      'report_template',
      null,
...
const mapReportCategory = (category: string): ReportType => {
  if (!category || category === 'analytics') return DEFAULT_REPORT_TYPE as ReportType;
...
const mapReportTypeToCategory = (type: ReportType): ReportCategory => {
  if (type === DEFAULT_REPORT_TYPE) return 'analytics';
  if (type === 'performance' || type === 'security' || type === 'seo') return type;
  return 'custom';
};
...
const mapReportRow = (
  ...
  if (row.user_id && String(row.user_id) !== String(userId)) return null;
...
const getReports = async (req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  ...
  try {
    ...
    const listResult = await query(
      `SELECT tr.id, tr.report_type, tr.format, tr.generated_at, tr.file_path, tr.file_size, tr.report_data, te.test_id, te.user_id, ri.status AS instance_status, ri.duration AS instance_duration, COALESCE(al.download_count, 0) AS download_count FROM test_reports tr LEFT JOIN test_executions te ON te.id = tr.execution_id LEFT JOIN report_instances ri ON ri.report_id = tr.id LEFT JOIN (SELECT report_id, COUNT(*)::int AS download_count FROM report_access_logs WHERE access_type = 'download' AND success = true GROUP BY report_id) al ON al.report_id = tr.id ${whereClause} ORDER BY tr.generated_at DESC LIMIT $${params.length + 1} OFFSET $${params.length + 2}`,
      [...params, limitNumber, offset]
    );
    const countResult = await query(
      `SELECT COUNT(*)::int AS total FROM test_reports tr LEFT JOIN test_executions te ON te.id = tr.execution_id ${whereClause}`,
      params
    );
    ...
    const byType = REPORT_TYPE_LIST.reduce<Record<string, number>>((acc, tv) => {
      acc[tv] = 0;
      return acc;
    }, {});
...
const updateTemplate = async (req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  const userId = req.user.id;
  ...
  try {
    ...
    if (!existing) return res.error(StandardErrorCode.NOT_FOUND, '报告模板不存在', undefined, 404);
...
const getTemplateVersions = async (req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  const { templateId } = req.params as { templateId: string };
  try {
    const result = await query(
      `SELECT id, version, action, created_by, created_at, data FROM data_record_versions WHERE record_id = $1 AND type = $2 ORDER BY version DESC`,
      [templateId, 'report_template']
    );
...
const previewTemplate = async (req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  const { templateId } = req.params as { templateId: string };
...
const deleteShareEmail = async (req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  const userId = req.user.id;
  const { id } = req.params;
  ...
  try {
    ...
    const ownership = await query(
      `SELECT rse.id, rse.status FROM report_share_emails rse LEFT JOIN test_reports tr ON tr.id = rse.report_id WHERE ${ownershipFilters.join(' AND ')}`,
      ownershipParams
    );
...
const getShareEmails = async (req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  const userId = req.user.id;
  const {
    reportId,
    shareId,
  ...
  if (String(onlyFailed) === 'true') filters.push(`rse.status = 'failed'`);
  if (filters.length > 0) filterClause = `AND ${filters.join(' AND ')}`;
  try {
    const result = await query(
      `SELECT rse.*, rs.share_token, rs.share_type, CASE WHEN rse.status = 'failed' AND rse.attempts < 3 THEN true ELSE false END AS can_retry, CASE WHEN rse.status = 'sent' THEN true ELSE false END AS can_delete FROM report_share_emails rse LEFT JOIN report_shares rs ON rs.id = rse.share_id LEFT JOIN test_reports tr ON tr.id = rse.report_id WHERE ${filters[0]} ${filterClause} ORDER BY rse.created_at DESC LIMIT $${params.length + 1} OFFSET $${params.length + 2}`,
      [...params, limitNumber, offset]
    );
    const countResult = await query(
      `SELECT COUNT(*)::int AS total FROM report_share_emails rse LEFT JOIN test_reports tr ON tr.id = rse.report_id WHERE ${filters[0]} ${filterClause}`,
      params
    );
    return res.success({
      data: result.rows || [],
...
const retryShareEmail = async (req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  const userId = req.user.id;
  const { id } = req.params;
  ...
  try {
    ...
    const ownership = await query(
      `SELECT rse.id, rse.status, rse.attempts FROM report_share_emails rse LEFT JOIN test_reports tr ON tr.id = rse.report_id WHERE ${ownershipFilters.join(' AND ')}`,
      ownershipParams
    );
    const ownedRow = ownership.rows?.[0];
    if (!ownedRow)
...
const getInstances = async (_req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  try {
    await ensureReportingInitialized();
...
const createShare = async (req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  const { id } = req.params;
  const userId = req.user.id;
  const {
    shareType = 'link',
  ...
  try {
    ...
    if (!reportRow) return res.error(StandardErrorCode.NOT_FOUND, '报告不存在', undefined, 404);
    ...
    const shareResult = await query(
      `INSERT INTO report_shares (report_id, shared_by, share_token, share_type, password_hash, allowed_ips, max_access_count, permissions, expires_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id, share_token`,
      [
        id,
        userId,
        shareToken,
    ...
    if (shareType === 'email') {
      ...
      await automatedReportingService.sendShareEmail({
        ...
        html:
          message ||
          `请使用以下链接访问报告：<a href="${shareUrl}">${shareUrl}</a>${password ? '<br/>该分享已设置访问密码。' : ''}`,
...
const getShareDetail = async (req: Request, res: ApiResponse, _next: NextFunction) => {
  const { token } = req.params;
  const { password } = req.query as { password?: string };
  const normalizedPassword = typeof password === 'string' ? password : '';
  const shareResult = await query(
    `SELECT rs.*, tr.file_path, tr.file_size, tr.format, COALESCE(al.download_count, 0) AS download_count FROM report_shares rs LEFT JOIN test_reports tr ON tr.id = rs.report_id LEFT JOIN (SELECT share_id, COUNT(*)::int AS download_count FROM report_access_logs WHERE access_type = 'download' AND success = true GROUP BY share_id) al ON al.share_id = rs.id WHERE rs.share_token = $1`,
    [token]
  );
  const shareRow = shareResult.rows[0];
  if (!shareRow) return res.error(StandardErrorCode.NOT_FOUND, '分享不存在', undefined, 404);
  ...
  if (!validation.allowed) {
    await automatedReportingService.recordAccess(String(shareRow.report_id), null, 'view', false, {
      shareId: shareRow.id,
...
const downloadShare = async (req: Request, res: ApiResponse, _next: NextFunction) => {
  const { token } = req.params;
  const { password = '' } = req.query as { password?: string };
  const shareResult = await query(
    `SELECT rs.*, tr.file_path, tr.file_size, tr.format FROM report_shares rs LEFT JOIN test_reports tr ON tr.id = rs.report_id WHERE rs.share_token = $1`,
    [token]
  );
  const shareRow = shareResult.rows[0];
  if (!shareRow) return res.error(StandardErrorCode.NOT_FOUND, '分享不存在', undefined, 404);
  ...
  await automatedReportingService.recordAccess(String(shareRow.report_id), null, 'download', true, {
    shareId: shareRow.id,
    userAgent: req.get('User-Agent') || undefined,
    ipAddress: req.ip,
  });
...
const getAccessLogs = async (req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  const userId = req.user.id;
  ...
  try {
    const result = await query(
      `SELECT ral.* FROM report_access_logs ral LEFT JOIN test_reports tr ON tr.id = ral.report_id WHERE ${filters[0]} ${filterClause} ORDER BY ral.accessed_at DESC LIMIT $${params.length + 1} OFFSET $${params.length + 2}`,
      [...params, limitNumber, offset]
    );
    const countResult = await query(
      `SELECT COUNT(*)::int AS total FROM report_access_logs ral LEFT JOIN test_reports tr ON tr.id = ral.report_id WHERE ${filters[0]} ${filterClause}`,
      params
    );
    return res.success({
      data: result.rows || [],
...
const getReportById = async (req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  const { id } = req.params;
  const userId = req.user.id;
  try {
    const reportResult = await query(
      `SELECT tr.id, tr.report_type, tr.format, tr.generated_at, tr.file_path, tr.file_size, tr.report_data, tr.workspace_id, te.test_id, te.user_id, ri.status AS instance_status, ri.duration AS instance_duration, ri.config_id AS instance_config_id, ri.template_id AS instance_template_id, ri.metadata AS instance_metadata, COALESCE(al.download_count, 0) AS download_count FROM test_reports tr LEFT JOIN test_executions te ON te.id = tr.execution_id LEFT JOIN report_instances ri ON ri.report_id = tr.id LEFT JOIN (SELECT report_id, COUNT(*)::int AS download_count FROM report_access_logs WHERE access_type = 'download' AND success = true GROUP BY report_id) al ON al.report_id = tr.id WHERE tr.id = $1`,
      [id]
    );
    const reportRow = reportResult.rows[0];
    const report = reportRow ? mapReportRow(reportRow, userId) : null;
    if (!report) return res.error(StandardErrorCode.NOT_FOUND, '报告不存在', undefined, 404);
    ...
    const shareResult = await query(
      `SELECT rs.id, rs.share_token, rs.share_type, rs.expires_at, rs.current_access_count, rs.max_access_count, rs.permissions, rs.last_accessed_at, COALESCE(al.download_count, 0) AS download_count FROM report_shares rs LEFT JOIN (SELECT share_id, COUNT(*)::int AS download_count FROM report_access_logs WHERE access_type = 'download' AND success = true GROUP BY share_id) al ON al.share_id = rs.id WHERE rs.report_id = $1 AND rs.is_active = true ORDER BY rs.created_at DESC`,
      [id]
    );
    ...
    const configResult = configId
      ? await query(
          `SELECT id, name, description, template_id, schedule, recipients, filters, format, delivery, enabled FROM report_configs WHERE id = $1`,
          [configId]
        )
      : { rows: [] as Record<string, unknown>[] };
    const templateResult = templateId
      ? await query(
          `SELECT id, name, description, report_type, default_format, is_public, is_system FROM report_templates WHERE id = $1`,
          [templateId]
        )
      : { rows: [] as Record<string, unknown>[] };
...
const createReport = async (req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  const userId = req.user.id;
  ...
  try {
    ...
    const matchedTemplate = templates.find(
      t => mapReportCategory(t.category) === reportRequest.type
    );
    if (!matchedTemplate)
      ...
      delivery: {
        ...
        settings: {
          email: reportRequest.schedule?.recipients?.length
            ? { subject: `${reportRequest.type} 报告`, body: '请查收报告附件', attachments: true }
            : undefined,
        },
      },
      enabled: true,
...
const downloadReport = async (req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  const { id } = req.params;
  const userId = req.user.id;
  try {
    const reportResult = await query(
      `SELECT tr.id, tr.file_path, tr.format, tr.report_data, tr.file_size, tr.generated_at, te.user_id FROM test_reports tr LEFT JOIN test_executions te ON te.id = tr.execution_id WHERE tr.id = $1`,
      [id]
    );
    const reportRow = reportResult.rows[0];
    if (!reportRow) return res.error(StandardErrorCode.NOT_FOUND, '报告不存在', undefined, 404);
...
const deleteReport = async (req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  const { id } = req.params;
  const userId = req.user.id;
  try {
    const reportResult = await query(
      `SELECT tr.file_path, tr.workspace_id, te.user_id FROM test_reports tr LEFT JOIN test_executions te ON te.id = tr.execution_id WHERE tr.id = $1`,
      [id]
    );
    const reportRow = reportResult.rows[0];
    if (!reportRow) return res.error(StandardErrorCode.NOT_FOUND, '报告不存在', undefined, 404);
    ...
    for (const cid of configIds) {
      const r = await query('SELECT 1 FROM report_instances WHERE config_id = $1 LIMIT 1', [cid]);
      if ((r.rows || []).length === 0)
        await query('DELETE FROM report_configs WHERE id = $1', [cid]);
    }
    for (const tid of templateIds) {
      const ri = await query('SELECT 1 FROM report_instances WHERE template_id = $1 LIMIT 1', [
        tid,
      ]);
      const rc = await query('SELECT 1 FROM report_configs WHERE template_id = $1 LIMIT 1', [tid]);
      if ((ri.rows || []).length === 0 && (rc.rows || []).length === 0)
        await query(
...
const getTemplates = async (req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  try {
    const { type } = req.query;
    await ensureReportingInitialized();
    let templates = (await automatedReportingService.getAllTemplates()) as ServiceReportTemplate[];
    if (type) templates = templates.filter(t => mapReportCategory(t.category) === String(type));
    const mapped = templates.map(t => ({
      id: t.id,
      name: t.name,
      type: mapReportCategory(t.category),
...
const createTemplate = async (req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  const userId = req.user.id;
  ...
  try {
    ...
    const templateId = await automatedReportingService.createTemplate({
      ...
      format: 'html',
      template,
      variables: (variables || []).map((v: ReportTemplate['variables'][number]) => ({
        name: v.name,
        type: (v.type || 'string') as 'string' | 'number' | 'boolean' | 'date' | 'array' | 'object',
        description: v.description || '',
        required: v.required ?? false,
        defaultValue: v.defaultValue,
      })),
      sections: [{ id: 'summary', name: '摘要', type: 'summary', order: 1, content: template }],
      styling: {
        theme: 'light',
        colors: {
          primary: '#1d4ed8',
...
const deleteTemplate = async (req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  const { templateId } = req.params;
  try {
    await ensureReportingInitialized();
    const deleted = await automatedReportingService.deleteTemplate(templateId);
    if (!deleted) {
      return res.error(StandardErrorCode.NOT_FOUND, '模板不存在');
    }
    return res.success(null, '模板已删除');
  } catch (error) {
    return res.error(
      StandardErrorCode.INTERNAL_SERVER_ERROR,
      '删除报告模板失败',
...
const getStatistics = async (req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  const userId = req.user.id;
  const { workspaceId } = req.query as { workspaceId?: string };
  try {
    if (workspaceId) await ensureWorkspacePermission(String(workspaceId), userId, 'read');
    const statsResult = await query(
      `SELECT tr.id, tr.report_type, tr.format, tr.file_size, tr.generated_at, tr.report_data, ri.status AS instance_status, ri.duration AS instance_duration, COALESCE(al.download_count, 0) AS download_count FROM test_reports tr LEFT JOIN test_executions te ON te.id = tr.execution_id LEFT JOIN report_instances ri ON ri.report_id = tr.id LEFT JOIN (SELECT report_id, COUNT(*)::int AS download_count FROM report_access_logs WHERE access_type = 'download' AND success = true GROUP BY report_id) al ON al.report_id = tr.id WHERE ${workspaceId ? 'tr.workspace_id = $1' : 'te.user_id = $1'}`,
      [workspaceId ?? userId]
    );
    ...
    reportRows.forEach(row => {
      const m = mapReportRow(row, userId);
      if (!m) return;
      byType[m.type] = (byType[m.type] || 0) + 1;
      byFormat[m.format] = (byFormat[m.format] || 0) + 1;
      byStatus[m.status] = (byStatus[m.status] || 0) + 1;
      totalSize += Number(row.file_size) || 0;
      if (row.instance_duration) durations.push(Number(row.instance_duration));
    });
...
const scheduleReport = async (req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  const { id } = req.params;
  ...
  try {
    await ensureReportingInitialized();
    const reportResult = await query(
      `SELECT tr.report_data, te.user_id FROM test_reports tr LEFT JOIN test_executions te ON te.id = tr.execution_id WHERE tr.id = $1`,
      [id]
    );
    const reportRow = reportResult.rows[0];
    if (!reportRow) return res.error(StandardErrorCode.NOT_FOUND, '报告不存在', undefined, 404);
    ...
    const configId = await automatedReportingService.createConfig({
      ...
      schedule: { type: 'recurring', cronExpression: mapFrequencyToCron(frequency ?? 'weekly') },
      ...
      filters: [],
      format: { type: 'pdf', options: {} },
      delivery: {
        method: recipients?.length ? 'email' : 'storage',
        settings: {
          email: recipients?.length
            ? { subject: '定时报表', body: '请查收报告附件', attachments: true }
            : undefined,
        },
      },
    ...
    return res.success(
      { enabled, frequency, recipients: recipients || [], configId },
      '定时任务设置成功'
    );
  } catch (error) {
    return res.error(
...
const exportReports = async (req: AuthRequest, res: ApiResponse, _next: NextFunction) => {
  const { format = 'json' } = req.query as { format?: string };
  const userId = req.user.id;
  try {
    const reportResult = await query(
      `SELECT tr.id, tr.report_type, tr.format, tr.generated_at, tr.file_size, te.test_id FROM test_reports tr LEFT JOIN test_executions te ON te.id = tr.execution_id WHERE te.user_id = $1`,
      [userId]
    );
    const rows = reportResult.rows || [];
    if (format === 'json')
      return res.downloadResponse(
    ...
    if (format === 'csv') {
      ...
      return res.downloadResponse(csvRows.join('\n'), 'reports.csv', 'text/csv');
    }
