...
const databaseStream = fs.createWriteStream(path.join(logsDir, 'database.log'), { flags: 'a' });
const performanceStream = fs.createWriteStream(path.join(logsDir, 'performance.log'), {
  flags: 'a',
});
...
const writeLog = (logEntry: Record<string, unknown>) => {
  combinedStream.write(JSON.stringify(logEntry) + '\n');
};
...
const securityLogger = (event: string, details: Record<string, unknown>, req?: RequestWithUser) => {
  ...
  securityStream.write(JSON.stringify(logEntry) + '\n');
...
const dbLogger = (query: string, duration: number, error: Error | null = null) => {
  ...
  databaseStream.write(JSON.stringify(logEntry) + '\n');
};

/**
 * 性能监控中间件
 */
const performanceMonitor = (req: RequestWithUser, res: Response, next: NextFunction) => {
  ...
  res.on('finish', () => {
    ...
    if (duration > 1000) {
      ...
      performanceStream.write(JSON.stringify(logEntry) + '\n');
    }
  });
