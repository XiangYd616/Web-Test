...
import { models as rawModels } from '../../database/pgModels';
...
type AuthType = 'none' | 'basic' | 'bearer' | 'oauth2' | 'apikey';

const models = rawModels as unknown as CollectionModels & {
  WorkspaceMember: ModelApi;
  Collection: ModelApi;
};
...
const ensureCollectionAccess = async (collectionId: string, userId: string) => {
  ...
  const workspaceId = collection.workspace_id ? String(collection.workspace_id) : '';
  if (!workspaceId) {
    return { error: '集合未关联工作空间' };
  }
  const member = await ensureWorkspaceMember(workspaceId, userId);
  if (!member) {
    return { error: '没有权限访问该集合' };
  }
  return { collection, member, workspaceId };
};
...
const ensureEnvironmentInWorkspace = async (environmentId: string, workspaceId: string) => {
  const envResult = await query('SELECT id FROM environments WHERE id = $1 AND workspace_id = $2', [
    environmentId,
    workspaceId,
  ]);
  return envResult.rows.length > 0;
};

const validateRequestInput = (data: RequestData, res: ApiResponse) => {
  const errors: ValidationError[] = [];
  const name = data?.name || '';
  if (!name || name.length > 255) {
...
const createCollection = async (req: AuthRequest, res: ApiResponse) => {
  try {
    ...
    if (collectionData.defaultEnvironmentId) {
      const valid = await ensureEnvironmentInWorkspace(
        collectionData.defaultEnvironmentId,
        workspaceId
      );
      if (!valid) {
        return res.notFound('环境不存在或不属于该工作空间');
      }
    }
...
const updateCollection = async (req: AuthRequest, res: ApiResponse) => {
  try {
    ...
    if (!workspaceId) {
      return res.error(StandardErrorCode.INVALID_INPUT, '集合未关联工作空间', undefined, 400);
    }

    // Partial validation for update: name is optional but if provided must be valid
    const updateErrors: ValidationError[] = [];
    const { name, description } = (req.body || {}) as { name?: string; description?: string };
    if (name !== undefined && (!name || name.length > 255)) {
      updateErrors.push({ field: 'name', message: '集合名称不能为空且长度不超过255' });
    }
    if (description !== undefined && description.length > 2000) {
      updateErrors.push({ field: 'description', message: '描述长度不能超过2000' });
    }
    if (updateErrors.length > 0) {
      return res.validationError(updateErrors);
    }

    if (req.body?.defaultEnvironmentId) {
      const valid = await ensureEnvironmentInWorkspace(
        String(req.body.defaultEnvironmentId),
        workspaceId
      );
      if (!valid) {
        return res.notFound('环境不存在或不属于该工作空间');
...
const setDefaultEnvironment = async (req: AuthRequest, res: ApiResponse) => {
  try {
    const { collectionId } = req.params;
    const { environmentId } = req.body as { environmentId?: string | null };
    ...
    if (typeof environmentId === 'string' && environmentId.trim().length > 0) {
      const valid = await ensureEnvironmentInWorkspace(environmentId, workspaceId);
      if (!valid) {
        return res.notFound('环境不存在或不属于该工作空间');
      }
    }

    const updated = await collectionManager.updateCollection(collectionId, {
      defaultEnvironmentId: environmentId || null,
    });

    return res.success(updated, '默认环境已更新');
  } catch (error) {
    Logger.error('设置默认环境失败', error, {
      userId: req.user.id,
      collectionId: req.params.collectionId,
    });
    return res.error(StandardErrorCode.INTERNAL_SERVER_ERROR, '设置默认环境失败', undefined, 500);
  }
};
...
const importPostmanCollection = async (req: AuthRequest, res: ApiResponse) => {
  try {
    ...
    const collection = await collectionManager.importPostmanCollection(
      postmanData as PostmanCollectionPayload,
      workspaceId,
      req.user.id
    );
...
const addRequest = async (req: AuthRequest, res: ApiResponse) => {
  try {
    ...
    const authType = requestData.auth?.type;
    const normalizedAuthType =
      authType &&
      (['none', 'basic', 'bearer', 'oauth2', 'apikey'] as AuthType[]).includes(authType as AuthType)
    ...
    const updated = await collectionManager.addRequestToCollection(collectionId, {
      ...
      auth: normalizedAuthType ? { type: normalizedAuthType } : undefined,
      metadata: {
        createdBy: req.user.id,
      },
    });
...
const updateRequest = async (req: AuthRequest, res: ApiResponse) => {
  try {
    ...
    const updated = await collectionManager.updateRequestInCollection(
      collectionId,
      requestId,
      req.body as Record<string, unknown>
    );
    return res.success(updated, '请求更新成功');
  } catch (error) {
    const message = error instanceof Error ? error.message : '更新请求失败';
    if (message === 'Request not found in collection') {
      return res.notFound('请求不存在');
    }
    Logger.error('更新请求失败', error, {
      userId: req.user.id,
      collectionId: req.params.collectionId,
    });
    return res.error(StandardErrorCode.INTERNAL_SERVER_ERROR, '更新请求失败', undefined, 500);
  }
};

const deleteRequest = async (req: AuthRequest, res: ApiResponse) => {
  try {
    const { collectionId, requestId } = req.params;
    const access = await ensureCollectionAccess(collectionId, req.user.id);
    if (access.error) {
    ...
    const updated = await collectionManager.deleteRequestFromCollection(collectionId, requestId);
    return res.success(updated, '请求删除成功');
  } catch (error) {
    const message = error instanceof Error ? error.message : '删除请求失败';
    if (message === 'Request not found in collection') {
      return res.notFound('请求不存在');
    }
    ...
    return res.error(StandardErrorCode.INTERNAL_SERVER_ERROR, '删除请求失败', undefined, 500);
  }
};

const deleteFolderFromCollection = async (req: AuthRequest, res: ApiResponse) => {
  try {
    ...
    const updated = await collectionManager.deleteFolderFromCollection(collectionId, folderId);
    return res.success(updated, '文件夹删除成功');
  } catch (error) {
    const message = error instanceof Error ? error.message : '删除文件夹失败';
    if (message === 'Folder not found in collection') {
      return res.notFound('文件夹不存在');
    }
    Logger.error('删除文件夹失败', error, {
      userId: req.user.id,
      collectionId: req.params.collectionId,
    });
    return res.error(StandardErrorCode.INTERNAL_SERVER_ERROR, '删除文件夹失败', undefined, 500);
  }
};
