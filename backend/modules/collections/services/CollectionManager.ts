...
import testResultRepository from '../../testing/repositories/testResultRepository';
import {
  insertExecutionLog,
  markCompletedWithLog,
  markFailedWithLog,
} from '../../testing/services/testLogService';
...
export type PostmanCollectionPayload = {
...
class CollectionManager {
  ...
  private parseJsonRecord<T>(value: unknown, fallback: T): T {
  ...
  private runPreRequestScript(
    request: ApiRequest,
    variables: Record<string, string>,
    script: string
  ) {
    const setVariable = (key: string, value: unknown) => {
      variables[key] = value === undefined ? '' : String(value);
    };
    this.runScriptInSandbox(script, {
      request,
      variables,
      setVariable,
    });
  ...
  private runScriptInSandbox(script: string, context: Record<string, unknown>) {
    const timeout = Number(process.env.COLLECTION_SCRIPT_TIMEOUT_MS || 1000);
    const safeTimeout = Number.isNaN(timeout) || timeout <= 0 ? 1000 : timeout;
    ...
    const runner = new vm.Script(script);
    runner.runInContext(sandbox, { timeout: safeTimeout });
  }
  ...
  async createCollection(
    ...
    const collectionRecord = await this.models.Collection.create({
      ...
      default_environment_id: collectionData.defaultEnvironmentId || null,
      definition: {
  ...
  async updateCollection(id: string, updates: Partial<ApiCollection>): Promise<ApiCollection> {
    ...
    const existingDefinition = this.parseJsonRecord<Record<string, unknown>>(
      existing.definition,
      {}
    );
    const existingMetadata = this.parseJsonRecord<Record<string, unknown>>(existing.metadata, {});
    const nextVersion = Number(existingMetadata.version || 1) + 1;
    const hasDefaultEnvironment = Object.prototype.hasOwnProperty.call(
      updates,
    ...
    const definition = {
      ...
      folders:
        updates.folders ?? (existingDefinition.folders as Array<Record<string, unknown>>) ?? [],
    };
    ...
    await this.models.Collection.update(
      {
        ...
        default_environment_id: hasDefaultEnvironment
          ? (updates.defaultEnvironmentId ?? null)
          : (existing.default_environment_id ?? null),
        definition,
        metadata,
        updated_at: new Date(),
      },
  ...
  async createRun(params: {
    ...
    const testId = params.options?.testId as string | undefined;
  ...
  async executeCollection(
    collectionId: string,
    environmentId: string,
    options: { runId?: string; environment?: Environment | null; testId?: string } = {}
  ): Promise<{
    ...
    const definition = this.parseJsonRecord<Record<string, unknown>>(
      collectionRecord.definition,
      {}
    );
    const requests = (definition.requests as ApiRequest[]) || [];
    const collectionAuth = (definition.auth as AuthConfig) || undefined;
    const collectionVariables = (definition.variables as Record<string, string>) || {};
    const resolvedEnvironmentId =
      environmentId || (collectionRecord.default_environment_id as string | undefined) || '';
    const environment =
      options.environment ||
      (resolvedEnvironmentId ? await this.getEnvironmentContext(resolvedEnvironmentId) : null);
    ...
    for (const request of requests) {
      ...
      const auth = this.resolveAuthConfig(request.auth, collectionAuth);
    ...
    if (options.testId) {
      const failedRequest = results.find(item => item.status === 'failed');
      const summary = {
        totalRequests: results.length,
        passedRequests: results.filter(item => item.status === 'completed').length,
        failedRequests: results.filter(item => item.status === 'failed').length,
        errorCount: results.filter(item => item.status === 'failed').length,
        logs,
      };
      const executionSeconds = Math.max(1, Math.round((Date.now() - start) / 1000));
      if (failedRequest) {
        await markFailedWithLog(options.testId, failedRequest.error || '集合执行失败', {
          summary,
          failedRequestId: failedRequest.requestId,
          failedRequestName: failedRequest.request.name,
        });
      } else {
        await markCompletedWithLog(options.testId, executionSeconds, { summary });
      }
    }
  ...
  private toApiCollection(record: CollectionRecord): ApiCollection {
    const definition = this.parseJsonRecord<Record<string, unknown>>(record.definition, {});
    ...
    return {
      ...
      defaultEnvironmentId: record.default_environment_id || null,
      variables: (definition.variables as Record<string, string>) || {},
      folders: (definition.folders as Array<Record<string, unknown>>) || [],
  ...
  async importPostmanCollection(
    postmanData: PostmanCollectionPayload,
    workspaceId: string,
    userId?: string
  ): Promise<ApiCollection> {
  ...
  async updateRequestInCollection(
    collectionId: string,
    requestId: string,
    ...
    requests[index] = { ...requests[index], ...updates, id: requestId };

    return this.updateCollection(collectionId, { requests });
  }
  ...
  async deleteRequestFromCollection(
    ...
    return this.updateCollection(collectionId, { requests: filtered });
  }

  async deleteFolderFromCollection(collectionId: string, folderId: string): Promise<ApiCollection> {
    const record = await this.models.Collection.findByPk(collectionId);
    if (!record) {
      throw new Error('Collection not found');
    }
    ...
    const folders = (definition.folders as Array<Record<string, unknown>>) || [];
    const filtered = folders.filter(f => f.id !== folderId);

    if (filtered.length === folders.length) {
      throw new Error('Folder not found in collection');
    }

    return this.updateCollection(collectionId, { folders: filtered } as Partial<ApiCollection>);
  }
  ...
  async executeRequest(
    request: ApiRequest,
    context: { variables?: Record<string, string>; auth?: AuthConfig; testId?: string } = {}
  ): Promise<ExecutionResult> {
    const startTime = Date.now();
    const testId = context.testId;
    const variables = this.mergeVariables({}, context.variables, request.variables);
    ...
    try {
      ...
      if (processedRequest.preRequestScript) {
        this.runPreRequestScript(processedRequest, variables, processedRequest.preRequestScript);
      }

      const response = await this.sendHttpRequest(processedRequest);

      if (processedRequest.postResponseScript) {
        this.runPostResponseScript(
          processedRequest,
          response,
          variables,
          processedRequest.postResponseScript
        );
      }
      ...
      if (testId) {
        await this.persistTestExecutionResult(testId, result, undefined);
      }
      ...
      if (testId) {
        ...
        await this.persistTestExecutionResult(
          testId,
          {
            requestId: request.id,
            status: 'failed',
  ...
  private async persistTestExecutionResult(
    testId: string,
    result: ExecutionResult,
    errorMessage?: string
  ): Promise<void> {
    const executionId = await this.resolveTestExecutionId(testId);
    const response = result.response;
    const tests = result.tests || [];
    const failedTests = tests.filter(test => !test.passed);
    const passedTests = tests.length - failedTests.length;
    const passed =
      failedTests.length === 0 && !errorMessage && (!response || response.status < 400);
    const summary = {
    ...
    if (executionId) {
      ...
      await testResultRepository.saveResult(
        executionId,
        summary,
        score,
        grade,
    ...
    const logMessage = passed ? '请求执行完成' : '请求执行失败';
    await insertExecutionLog(testId, logLevel, logMessage, summary);
  }

  private async resolveTestExecutionId(testId: string): Promise<number | null> {
    try {
      const result = await query('SELECT id FROM test_executions WHERE test_id = $1', [testId]);
      const row = result.rows?.[0] as { id?: number } | undefined;
      return row?.id ?? null;
    } catch (error) {
      Logger.warn('获取测试执行记录失败', { testId, error });
      return null;
    }
  }
  ...
  private processRequest(
    request: ApiRequest,
    context: { variables?: Record<string, string>; auth?: AuthConfig } = {}
  ): ApiRequest {
    const processedRequest = { ...request };
    const variables = context.variables || {};

    processedRequest.url = this.replaceVariables(processedRequest.url, variables);
    ...
    if (processedRequest.params) {
      processedRequest.params = this.replaceVariablesInObject(
    ...
    if (processedRequest.body) {
      processedRequest.body = this.replaceVariablesInObject(processedRequest.body, variables);
    }

    if (context.auth) {
      this.applyAuthToRequest(processedRequest, context.auth, variables);
    }
  ...
  private resolveAuthConfig(requestAuth?: AuthConfig, collectionAuth?: AuthConfig) {
    if (requestAuth?.type === 'none') {
      return undefined;
    }
    return requestAuth || collectionAuth;
  }
  ...
  private applyAuthToRequest(
    ...
    if (auth.type === 'basic' && auth.basic) {
      const username = this.replaceVariables(auth.basic.username, variables);
      const password = this.replaceVariables(auth.basic.password, variables);
      const token = Buffer.from(`${username}:${password}`).toString('base64');
      headers.Authorization = `Basic ${token}`;
    }

    if (auth.type === 'bearer' && auth.bearer?.token) {
      const token = this.replaceVariables(auth.bearer.token, variables);
      headers.Authorization = `Bearer ${token}`;
    }

    if (auth.type === 'apikey' && auth.apikey) {
      const key = this.replaceVariables(auth.apikey.key, variables);
      const value = this.replaceVariables(auth.apikey.value, variables);
      if (auth.apikey.addTo === 'header') {
        headers[key] = value;
      } else if (auth.apikey.addTo === 'query') {
        params[key] = value;
      } else if (auth.apikey.addTo === 'cookie') {
        const existing = headers.Cookie ? `${headers.Cookie}; ` : '';
        headers.Cookie = `${existing}${key}=${value}`;
      }
    }
