...
import comparisonRepository from '../testing/repositories/comparisonRepository';

/**
 * 测试结果对比分析器
 *
 * 文件路径: backend/modules/utils/ComparisonAnalyzer.ts
 * 创建时间: 2025-11-14
 *
 * 功能:
 * - 历史数据对比
...
class ComparisonAnalyzer {
  ...
  async compareToBenchmark(testResult: Record<string, unknown>, benchmarkType: string) {
    ...
    const comparison = baseline ? this.buildBaselineComparison(current, baseline) : null;
    const payload: BenchmarkComparisonResult & { comparisonId?: string } = {
      benchmark,
      current,
      comparison,
    ...
    const userId = typeof testResult.userId === 'string' ? testResult.userId : undefined;
    const explicitName =
      typeof testResult.comparisonName === 'string' && testResult.comparisonName.trim().length > 0
        ? testResult.comparisonName.trim()
        : undefined;
    const executionIdRaw = testResult.executionId;
    const executionIds = Array.isArray(testResult.executionIds)
      ? (testResult.executionIds as number[])
      : typeof executionIdRaw === 'number'
        ? [executionIdRaw]
        : [];

    if (userId && executionIds.length > 0) {
      const comparisonName =
        explicitName || `${benchmark.name} - ${testResult.testId || 'unknown'}`;
      const comparisonId = await this.saveComparison({
        userId,
        comparisonName,
        executionIds,
        comparisonType: 'benchmark',
        comparisonData: payload as unknown as Record<string, unknown>,
      });
      if (comparisonId) {
  ...
  async getAvailableBenchmarks(testType?: string) {
    const service = await this.getBenchmarkService();
    const benchmarks = (await service.getAllBenchmarks()) as BenchmarkConfig[];
  ...
  async generateComparisonSummary(comparisons: unknown[], groupBy?: string) {
    ...
    items.forEach(item => {
      ...
      const changes = Object.values(item.comparison?.changes || {}) as MetricChange[];
      const changeSum = changes.reduce((sum, change) => sum + (change.percentage || 0), 0);
      const changeCount = changes.length;

      totalChangePercent += changeSum;
      totalChangeMetrics += changeCount;

      if (groupKey) {
        const group = ensureGroup(groupKey);
        group.total += 1;
        if (trend === 'improving') group.improving += 1;
        else if (trend === 'degrading') group.degrading += 1;
        else group.stable += 1;
        group.averageChangePercent += changeSum;
        group.metrics += changeCount;
      }
    });

    summary.averageChangePercent = totalChangeMetrics
      ? Number((totalChangePercent / totalChangeMetrics).toFixed(2))
      : 0;

    Object.values(summary.groups).forEach(group => {
      group.averageChangePercent = group.metrics
        ? Number((group.averageChangePercent / group.metrics).toFixed(2))
        : 0;
    });
  ...
  async getComparisonMetrics(testType?: string) {
    const benchmarks = (await this.getAvailableBenchmarks(testType)) as BenchmarkConfig[];
    const metrics = new Set<string>();

    benchmarks.forEach(benchmark => {
      benchmark.metrics.forEach((metric: string) => metrics.add(metric));
    });
  ...
  async exportComparisonReport(comparisonId: string, format = 'json', options?: unknown) {
    ...
    if (comparisons.length === 0) {
      ...
      const stored = await this.loadStoredComparison(comparisonId);
      if (!stored) {
    ...
    const summary = payload.includeSummary
      ? await this.generateComparisonSummary(comparisons, 'benchmark')
      : undefined;
    const report = {
    ...
    if (format === 'csv') {
      ...
      comparisons.forEach(item => {
        const metrics = (item.benchmark.metrics || []) as string[];
  ...
  async saveComparison(payload: SaveComparisonPayload): Promise<string | null> {
    if (!payload.executionIds.length) {
      return null;
    }
    return comparisonRepository.insertComparison(payload);
  }
  ...
  private buildCurrentPerformanceData(
    ...
    benchmark.metrics.forEach((metric: string) => {
