...
class EnvironmentManager {
  ...
  getVariable(key: string, options: GetVariableOptions = {}) {
    ...
    // 4. 检查全局变量（仅在 environment 或 global 作用域下查找）
    if ((context === 'environment' || context === 'global') && this.globalVariables.has(key)) {
  ...
  encryptValue(value: string) {
    ...
    ).createCipher('aes192', this.options.encryptionKey) as {
      update: (
        value: string,
  ...
  decryptValue(encryptedValue: string) {
    try {
      const decipher = (
        crypto as unknown as { createDecipher: (algo: string, key: string) => unknown }
      ).createDecipher('aes192', this.options.encryptionKey) as {
        update: (
          value: string,
          inputEncoding: BufferEncoding,
          outputEncoding: BufferEncoding
  ...
  async loadEnvironment(environmentId: string): Promise<null> {
    console.warn('File persistence for environments deprecated, use DB tables');
    void environmentId;
    return null;
  }
   ...
   * 更新环境基本信息
   */
  async updateEnvironment(environmentId: string, updates: { name?: string; description?: string }) {
    const environmentRecord = await this.fetchEnvironmentRecord(environmentId);
    if (!environmentRecord) {
      throw new Error(`环境不存在: ${environmentId}`);
    }
    ...
    await this.updateEnvironmentRecord(environmentId, dbUpdates);
   ...
   * 删除环境变量
   */
  async deleteVariable(environmentId: string, key: string) {
    const environmentRecord = await this.fetchEnvironmentRecord(environmentId);
    if (!environmentRecord) {
      throw new Error(`环境不存在: ${environmentId}`);
    }

    await query('DELETE FROM environment_variables WHERE environment_id = $1 AND key = $2', [
      environmentId,
      key,
    ]);
    ...
    const cached = this.environments.get(environmentId);
    if (cached) {
      cached.variables = cached.variables.filter(v => v.key !== key);
      cached.updatedAt = new Date().toISOString();
    }
