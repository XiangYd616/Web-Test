...
type TestSession = {
  ...
  socketId: string;
  endTime?: Date;
  results?: import('../../../shared/types/testEngine.types').BaseTestResult;
};
...
class TestEventsHandler {
  ...
  async handleStressTestStart(socket: SocketLike, config: Record<string, unknown>) {
    const testId = (config as { testId?: string }).testId;
    if (!testId) {
      socket.emit('stress:error', {
        testId: 'unknown',
        error: '缺少 testId，无法启动压力测试',
        timestamp: new Date().toISOString(),
      });
      return;
    }
  ...
  emitTestProgress(testId: string, payload: Record<string, unknown>) {
    this.io.to(testId).emit('test-progress', {
      testId,
      ...payload,
      timestamp: new Date().toISOString(),
    });
  ...
  emitTestComplete(testId: string, result: Record<string, unknown>) {
    this.io.to(testId).emit('test-completed', {
      testId,
      result,
      timestamp: new Date().toISOString(),
    });
  ...
  emitTestError(testId: string, error: Record<string, unknown>) {
    this.io.to(testId).emit('test-error', {
      testId,
      error,
      timestamp: new Date().toISOString(),
    });
...
let activeHandler: TestEventsHandler | null = null;

const registerTestEventsHandler = (handler: TestEventsHandler) => {
  activeHandler = handler;
};

const emitTestProgress = (testId: string, payload: Record<string, unknown>) => {
  activeHandler?.emitTestProgress(testId, payload);
};

const emitTestComplete = (testId: string, result: Record<string, unknown>) => {
  activeHandler?.emitTestComplete(testId, result);
};

const emitTestError = (testId: string, error: Record<string, unknown>) => {
  activeHandler?.emitTestError(testId, error);
};
