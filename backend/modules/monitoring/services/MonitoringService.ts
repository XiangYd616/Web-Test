...
class MonitoringService extends EventEmitter {
    ...
    try {
      ...
      try {
        ...
        if (targets.length > 0) {
          ...
          logger.debug(`使用基础查询加载了 0 个监控目标`);
        }
  ...
  async performPerformanceCheck(target: MonitoringTarget, timeout: number) {
    ...
    const timing = await this.measureNetworkTimings(target.url, timeout).catch((): null => null);
  ...
  async performHealthCheck() {
    try {
      ...
      if (this.activeMonitors.size > 0 || stats.overallHealth !== 'healthy') {
        logger.debug(
          `监控服务健康检查: ${stats.activeMonitors} 个活跃监控, 状态: ${stats.overallHealth}`
        );
      }

      if (stats.overallHealth !== 'healthy' && this.activeMonitors.size > 0) {
  ...
  determineOverallHealth(stats: Record<string, unknown>) {
    ...
    if (resources?.memory?.usage && resources.memory.usage > 95) {
      return 'degraded';
    }
  ...
  async saveReportRecord(
    ...
    try {
      ...
      if (errCode === '42P01' || errMsg.includes('no such table')) {
  ...
  async createReportsTable() {
    ...
    // 索引单独创建，兼容 SQLite（不支持 CREATE TABLE 内联 INDEX）
    await this.dbPool.query(
      'CREATE INDEX IF NOT EXISTS idx_monitoring_reports_user ON monitoring_reports(user_id)'
    );
    await this.dbPool.query(
      'CREATE INDEX IF NOT EXISTS idx_monitoring_reports_created ON monitoring_reports(created_at DESC)'
    );
