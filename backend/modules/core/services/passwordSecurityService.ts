...
class PasswordSecurityService {
  ...
  async changePassword(userId: string, currentPassword: string, newPassword: string) {
    try {
      ...
      const user = userResult.rows[0] as { password_hash: string; email: string; username: string };
  ...
  async recordLoginFailure(userId: string, ipAddress: string, userAgent: string) {
    try {
      ...
      if (userResult.rows.length > 0) {
        const { login_attempts, locked_until } = userResult.rows[0] as {
  ...
  async isAccountLocked(userId: string): Promise<AccountLockStatus> {
    try {
      ...
      const { locked_until } = result.rows[0] as { locked_until: string | null };
  ...
  async resetPasswordWithToken(token: string, newPassword: string) {
    try {
      ...
      // 获取当前密码用于历史记录
      const currentPasswordResult = await query('SELECT password_hash FROM users WHERE id = $1', [
        userId,
      ]);
      ...
      await transaction(
        async (client: { query: (sql: string, params?: unknown[]) => Promise<unknown> }) => {
          ...
          // 保存密码历史
          const currentPasswordHash = (currentPasswordResult.rows[0] as { password_hash: string })
            ?.password_hash;
          if (currentPasswordHash) {
            await svc.savePasswordHistory(userId, currentPasswordHash);
          }
          ...
          // 重置登录尝试和解锁账户
          await svc.resetLoginAttempts(userId);
        }
      );
    ...
    try {
      ...
      for (const user of warningResult.rows) {
        ...
        const daysUntilExpiry =
          ...
          Math.floor(
            (Date.now() - new Date(u.password_changed_at).getTime()) / (1000 * 60 * 60 * 24)
          );

        await this.sendPasswordExpiryWarning(u.email, u.username, daysUntilExpiry);

        // 更新警告时间
        await query('UPDATE users SET last_password_warning = NOW() WHERE id = $1', [u.id]);
      }
      ...
      for (const user of expiredResult.rows) {
        const u = user as { id: string; email: string; username: string };
        await query('UPDATE users SET password_expired = true WHERE id = $1', [u.id]);

        await this.sendPasswordExpiredNotification(u.email, u.username);
      }
  ...
  async sendAccountLockNotification(userId: string, lockedUntil: Date) {
    try {
      ...
      if (userResult.rows.length > 0) {
        const { email, username } = userResult.rows[0] as { email: string; username: string };
