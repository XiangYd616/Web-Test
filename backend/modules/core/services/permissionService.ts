...
const ROLE_PERMISSIONS: Record<string, string[]> = {
  ...
  // 高级用户 - 注意：付费等级应使用 UserPlan，此处保留兼容
  premium: [
    PERMISSIONS.USER_READ,
    PERMISSIONS.TEST_CREATE,
    PERMISSIONS.TEST_READ,
...
class PermissionService {
   ...
   */
  async getUserPermissions(userId: number | string) {
    try {
      // 检查缓存
      ...
      // 从数据库获取用户信息
      const userResult = (await query('SELECT id, role, is_active FROM users WHERE id = $1', [
        userId,
      ])) as DbQueryResult<{ role: string; is_active: boolean }>;
      ...
      try {
        const customPermissionsResult = (await query(
          ...
          [userId]
        )) as DbQueryResult<{ permission: string }>;

        const customPermissions = customPermissionsResult.rows.map(row => row.permission);
        permissions = [...new Set([...permissions, ...customPermissions])] as string[];
      } catch (error) {
        void error;
        // 如果没有自定义权限表，忽略错误
  ...
  async isAdmin(userId: number | string) {
    try {
      const userResult = (await query('SELECT role FROM users WHERE id = $1 AND is_active = true', [
        userId,
      ])) as DbQueryResult<{ role: string }>;

      if (userResult.rows.length === 0) {
        return false;
      }

      return ['admin', 'superadmin'].includes(userResult.rows[0].role);
    } catch (error) {
  ...
  isValidPermission(permission: string) {
    return (Object.values(PERMISSIONS) as string[]).includes(permission);
  }
   ...
   * @param {number} userId - 用户ID
   */
  clearUserPermissionCache(userId: number | string) {
    const cacheKey = `user_permissions_${userId}`;
   ...
   */
  async removeUserPermission(userId: number | string, permission: string) {
    try {
      await query(
...
export { PERMISSIONS, PermissionService, ROLE_PERMISSIONS };
