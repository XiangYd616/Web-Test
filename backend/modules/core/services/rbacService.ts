...
class RBACService {
  ...
  async updateRole(roleId: string, updates: Record<string, unknown>, updatedBy: string) {
    try {
      // 获取当前角色信息
      const currentResult = await query('SELECT * FROM roles WHERE id = $1', [roleId]);
  ...
  async deleteRole(roleId: string, deletedBy: string) {
    try {
      ...
      if (Number.parseInt((userRoleResult.rows[0] as { count: string }).count, 10) > 0) {
  ...
  async assignRole(
    ...
    try {
      // 检查角色是否存在
      const roleResult = await query('SELECT * FROM roles WHERE id = $1 AND is_active = true', [
        roleId,
      ]);
      if (roleResult.rows.length === 0) {
