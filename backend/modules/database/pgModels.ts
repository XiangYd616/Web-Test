...
const buildWhereClause = (where: Record<string, unknown> = {}, paramOffset = 0) => {
  ...
  Object.entries(where).forEach(([key, value]) => {
    ...
    if (typeof value === 'object' && value) {
      ...
      if (searchValue !== undefined) {
        ...
        clauses.push(`${key} ILIKE $${paramOffset + values.length}`);
        return;
      }
    }
    values.push(value);
    clauses.push(`${key} = $${paramOffset + values.length}`);
  });
...
const createModel = (table: string, primaryKey = 'id') => {
  ...
  Object.assign(modelApi, {
        ...
        if (includeWorkspace) {
          const result = await query(
            `SELECT wm.*,
                    ...
                    w.visibility as w_visibility, w.owner_id as w_owner_id,
                    w.metadata as w_metadata, w.created_at as w_created_at,
          ...
          return result.rows.map(row => {
            ...
            r.workspace = {
              ...
              visibility: r.w_visibility,
              owner_id: r.w_owner_id,
              metadata: r.w_metadata,
    ...
    async update(data: Record<string, unknown>, options: UpdateOptions = {}) {
      ...
      const params = keys.map(key => data[key]);
      const { clause, values } = buildWhereClause(options.where, keys.length);
      const returningClause = options.returning ? 'RETURNING *' : '';
