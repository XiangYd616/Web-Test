...
class WebSocketManager extends EventEmitter {
  ...
  private async processMessage(message: WebSocketMessage): Promise<WebSocketMessage> {
    ...
    if (message.encrypted && this.isEncryptedPayload(message.data)) {
      message.data = await this.decryptMessage(message.data);
    }

    if (this.isCompressedPayload(message.data)) {
      message.data = await this.decompressMessage(message.data);
    }

    if (
      !this.isCompressedPayload(message.data) &&
      JSON.stringify(message.data).length > this.config.messages.compressionThreshold
    ) {
   ...
   * 加密消息
   */
  private async encryptMessage(data: unknown): Promise<EncryptedPayload | unknown> {
  ...
  private async decryptMessage(data: EncryptedPayload | unknown): Promise<unknown> {
   ...
   * 压缩消息
   */
  private async compressMessage(data: unknown): Promise<CompressedPayload> {
  ...
  private async decompressMessage(data: CompressedPayload | unknown): Promise<unknown> {
    if (!this.isCompressedPayload(data)) {
      return data;
    }
  ...
  private resolveEncryptionKey(): Buffer | null {
    const rawKey =
      process.env.WS_ENCRYPTION_KEY || process.env.JWT_SECRET || process.env.APP_SECRET || '';
    if (!rawKey) {
      return null;
    }
  ...
  private isEncryptedPayload(data: unknown): data is EncryptedPayload {
    return (
      typeof data === 'object' &&
      data !== null &&
      '__encrypted' in data &&
      (data as EncryptedPayload).__encrypted === true
    );
  }

  private isCompressedPayload(data: unknown): data is CompressedPayload {
    return (
      typeof data === 'object' &&
      data !== null &&
      '__compressed' in data &&
      (data as CompressedPayload).__compressed === true
    );
  }
  ...
  private updateStatistics(type: string, message: WebSocketMessage): void {
    ...
    if (type === 'message_sent' || type === 'message_received') {
      ...
      this.pruneMessageTimestamps(now);
    }
  ...
  private calculateMessagesPerSecond(): number {
    ...
    return this.messageTimestamps.length / (windowMs / 1000);
  }

  private pruneMessageTimestamps(now: number): void {
    const windowMs = 60000;
    while (this.messageTimestamps.length > 0 && now - this.messageTimestamps[0] > windowMs) {
