...
class DataCleanupManager {
  ...
  private startScheduledTasks(): void {
    ...
    task.start();
    console.log(`清理调度任务启动完成: ${this.scheduledTasks.size} 个任务`);
  }
  ...
  private getDefaultRetentionPolicies(): RetentionPolicy[] {
    if (process.env.CLEANUP_SEED_POLICIES === 'false') {
      return [];
    }
    return [
      {
  ...
  private async createCleanupTables(): Promise<void> {
    ...
    await query(`CREATE INDEX IF NOT EXISTS idx_cleanup_jobs_status ON cleanup_jobs(status)`, []);
    await query(
