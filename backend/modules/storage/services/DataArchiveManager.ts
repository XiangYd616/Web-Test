...
class DataArchiveManager {
  ...
  private async startScheduledTasks(): Promise<void> {
    try {
      const policies = await this.getAllArchivePolicies();
      const enabledPolicies = policies.filter(policy => policy.enabled);
      if (enabledPolicies.length === 0) {
        console.warn('未启用归档策略，跳过归档调度启动');
        return;
      }
      for (const policy of enabledPolicies) {
        await this.schedulePolicy(policy.id);
      }
      console.log(`归档调度任务启动完成: ${this.scheduledTasks.size} 个策略`);
    } catch (error) {
      console.error('启动归档调度任务失败:', error);
    }
  }
  ...
  private async loadJobs(): Promise<void> {
    try {
      ...
      if (code === '42P01' || msg.includes('no such table')) {
        await this.ensureTables();
      } else {
        throw error;
      }
  ...
  private async ensureTables(): Promise<void> {
    ...
    await query(`CREATE INDEX IF NOT EXISTS idx_archive_jobs_status ON archive_jobs(status)`, []);
    await query(
