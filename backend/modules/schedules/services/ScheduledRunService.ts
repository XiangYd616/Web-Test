...
import type { Environment as CollectionEnvironment } from '../../collections/services/CollectionManager';
import CollectionManager from '../../collections/services/CollectionManager';
import { query } from '../../config/database';
import { EnvironmentManager } from '../../environments/services/EnvironmentManager';
...
class ScheduledRunService {
  ...
  async updateSchedule(
    ...
    const schedule = await this.fetchScheduleById(scheduleId, workspaceId);
    if (!schedule) {
      throw new Error('Schedule not found');
    }

    await this.updateScheduleRecord(
      scheduleId,
      {
        name: updates.name,
    ...
    const updatedSchedule = await this.fetchScheduleById(scheduleId, workspaceId);
    if (!updatedSchedule) {
      throw new Error('Schedule not found');
    }
  ...
  async deleteSchedule(scheduleId: string, workspaceId?: string): Promise<boolean> {
    ...
    await this.deleteScheduleRecord(scheduleId, workspaceId);
    return true;
  }
  ...
  async executeSchedule(
    ...
    const schedule = await this.fetchScheduleById(scheduleId, options.workspaceId);
    if (!schedule) {
      throw new Error('Schedule not found');
    }
   ...
   * 取消执行
   */
  async cancelExecution(executionId: string, workspaceId?: string): Promise<boolean> {
    ...
    if (workspaceId) {
      const schedule = await this.fetchScheduleById(execution.scheduledRunId, workspaceId);
      if (!schedule) {
        return false;
      }
    }
  ...
  private async executeCollectionRun(
    ...
    try {
      ...
      const result = await this.collectionManager.executeCollection(
        ...
        { environment: environment as unknown as CollectionEnvironment | null }
      );

      const runResult: ScheduledRunResult = {
        id: this.generateId(),
        scheduledRunId: schedule.id,
        status: result.summary.failedRequests === 0 ? 'success' : 'failed',
        startedAt: startTime,
        completedAt: new Date(),
        duration: result.duration,
        totalRequests: result.summary.totalRequests,
        passedRequests: result.summary.passedRequests,
        failedRequests: result.summary.failedRequests,
        errorCount: result.summary.errorCount,
        logs: result.summary.logs,
  ...
  private async fetchScheduleById(
    ...
    const params: Array<string | number> = [scheduleId];
    const whereClause = workspaceId ? 'WHERE id = $1 AND workspace_id = $2' : 'WHERE id = $1';
    if (workspaceId) {
      params.push(workspaceId);
    }
    const result = await query(`SELECT * FROM scheduled_runs ${whereClause}`, params);
  ...
  private async updateScheduleRecord(
    ...
    await query(`UPDATE scheduled_runs SET ${setClause} ${whereClause}`, values);
  }

  private async deleteScheduleRecord(scheduleId: string, workspaceId?: string) {
    if (workspaceId) {
      await query('DELETE FROM scheduled_runs WHERE id = $1 AND workspace_id = $2', [
        scheduleId,
        workspaceId,
      ]);
      return;
