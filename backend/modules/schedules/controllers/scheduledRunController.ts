...
type ScheduledRunServiceLike = {
  ...
  deleteSchedule: (scheduleId: string, workspaceId?: string) => Promise<boolean>;
  ...
  cancelExecution: (executionId: string, workspaceId?: string) => Promise<boolean>;
};
...
const updateScheduledRun = async (req: AuthRequest, res: ApiResponse) => {
  try {
    ...
    const service = getScheduledRunService();
    await service.updateSchedule(scheduledRun.id, updates, scheduledRun.workspace_id || undefined);
...
const deleteScheduledRun = async (req: AuthRequest, res: ApiResponse) => {
  try {
    ...
    const service = getScheduledRunService();
    await service.deleteSchedule(scheduledRun.id, scheduledRun.workspace_id || undefined);
...
const cancelExecution = async (req: AuthRequest, res: ApiResponse) => {
  try {
    ...
    const service = getScheduledRunService();
    const cancelled = await service.cancelExecution(executionId, workspaceId);
    if (!cancelled) {
...
const getSchedulingStatistics = async (_req: AuthRequest, res: ApiResponse) => {
  try {
    ...
    const statistics = await _scheduledRunService.getStatistics(workspaceId);
...
const startScheduledRun = async (req: AuthRequest, res: ApiResponse) => {
  try {
    ...
    await service.updateSchedule(
      scheduledRun.id,
      { status: 'active' },
      scheduledRun.workspace_id || undefined
    );
    const refreshed = await fetchScheduledRunById(scheduledRun.id);
