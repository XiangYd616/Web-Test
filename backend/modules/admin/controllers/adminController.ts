...
import { healthCheck as dbHealthCheck, getStats as getDbStats, query } from '../../config/database';
import { getApiStatsSnapshot } from '../../middleware/logger';
import type { ApiResponse, AuthenticatedRequest } from '../../types';
import { toOptionalDate } from '../../utils/dateUtils';
import { getSecurityEvents, SecurityEventType, SecuritySeverity } from '../../utils/securityLogger';
...
const buildSystemConfig = (map: Map<string, Record<string, unknown>>) => {
  ...
  return {
    ...
    testing: {
      ...
      quotaDailyFree: Number(getConfigValue(map, 'test_quota_daily_free', 10)) || 10,
      quotaMonthlyFree: Number(getConfigValue(map, 'test_quota_monthly_free', 100)) || 100,
      quotaDailyPremium: Number(getConfigValue(map, 'test_quota_daily_premium', 100)) || 100,
      quotaMonthlyPremium: Number(getConfigValue(map, 'test_quota_monthly_premium', 1000)) || 1000,
      ...
      webHeavyTestEnabled: getConfigValue(map, 'web_heavy_test_enabled', true) !== false,
    },
    monitoring: {
      uptimeCheckInterval: Number(getConfigValue(map, 'uptime_check_interval', 300)) || 300,
      dashboardRefreshSeconds: Number(getConfigValue(map, 'dashboard_refresh_seconds', 30)) || 30,
      alertThresholds: {
      ...
      queueThresholds: {
        warning: Number(getConfigValue(map, 'queue_warning_threshold', 20)) || 20,
        critical: Number(getConfigValue(map, 'queue_critical_threshold', 100)) || 100,
      },
      retentionPeriods: {
...
class AdminController {
    ...
    try {
      ...
      const testSummary = await query(
        `SELECT
          ...
          COALESCE(AVG(execution_time), 0) AS avg_duration
        FROM test_executions`
      );

      const typeStats = await query(
        `SELECT engine_type AS type, COUNT(*) AS count
         FROM test_executions
    ...
    try {
      ...
      const runningTestsResult = await query(
        `SELECT
          ...
          SUM(CASE WHEN status IN ('pending', 'queued', 'preparing') THEN 1 ELSE 0 END) AS queued
         FROM test_executions`
      );
      ...
      return res.success({
        ...
        metrics: {
          ...
          database: {
            connections:
              (dbConnections as Record<string, unknown>).total_connections ||
              dbStats?.poolStats?.totalCount ||
              0,
            queryTime: 0,
            slowQueries: 0,
            size: dbSizeBytes,
    ...
    try {
      ...
      const listSql = `
        SELECT
          ...
          u.created_at,
          u.updated_at,
          COALESCE((SELECT COUNT(*) FROM test_executions te WHERE te.user_id = u.id), 0) AS test_count,
          (SELECT MAX(created_at) FROM test_executions te WHERE te.user_id = u.id) AS last_activity,
          (SELECT ip_address FROM user_sessions us WHERE us.user_id = u.id ORDER BY us.last_activity_at DESC LIMIT 1) AS ip_address,
          (SELECT user_agent FROM user_sessions us WHERE us.user_id = u.id ORDER BY us.last_activity_at DESC LIMIT 1) AS user_agent
        FROM users u
        ${whereClause}
  ...
  async getTests(req: AdminRequest, res: ApiResponse, _next: NextFunction) {
    try {
      ...
      const countSql = `SELECT COUNT(*) AS total FROM test_executions te ${whereClause}`;
      const countResult = await query(countSql, params);
      const listResult = await query(listSql, [...params, limit, offset]);
      const total = Number(countResult.rows[0]?.total) || 0;
  ...
  async getLogs(req: AdminRequest, res: ApiResponse, _next: NextFunction) {
    try {
      ...
      const countSql = `SELECT COUNT(*) AS total FROM test_logs ${whereClause}`;
      const countResult = await query(countSql, params);
      const listResult = await query(listSql, [...params, limit, offset]);
      const total = Number(countResult.rows[0]?.total) || 0;
  ...
  async getSecurityEvents(req: AdminRequest, res: ApiResponse, _next: NextFunction) {
    try {
      const { page, limit } = parsePagination(req);
      const { search, type, severity, startTime, endTime } = req.query as Record<string, string>;
      const parsedType = Object.values(SecurityEventType).includes(type as SecurityEventType)
        ? (type as SecurityEventType)
        : undefined;
      const parsedSeverity = Object.values(SecuritySeverity).includes(severity as SecuritySeverity)
      ...
      const result = await getSecurityEvents({
        ...
        startTime: toOptionalDate(startTime),
        endTime: toOptionalDate(endTime),
      });

      return res.success(result.items, undefined, 200, {
        pagination: {
          page,
          limit,
          total: result.total,
      ...
      return handleControllerError(res, error, '获取安全事件失败');
    }
  }
  ...
  async getSystemConfig(_req: AdminRequest, res: ApiResponse, _next: NextFunction) {
    try {
      ...
      if (err?.code === '42P01' || err?.message?.includes('no such table')) {
  ...
  async updateSystemConfig(req: AdminRequest, res: ApiResponse, _next: NextFunction) {
    try {
      ...
      await upsertConfig('test_quota_daily_free', testing.quotaDailyFree, 'performance');
      await upsertConfig('test_quota_monthly_free', testing.quotaMonthlyFree, 'performance');
      await upsertConfig('test_quota_daily_premium', testing.quotaDailyPremium, 'performance');
      await upsertConfig('test_quota_monthly_premium', testing.quotaMonthlyPremium, 'performance');
      await upsertConfig(
      ...
      if (testing.webHeavyTestEnabled !== undefined) {
        await upsertConfig('web_heavy_test_enabled', testing.webHeavyTestEnabled, 'features');
      }
      ...
      const monitoring = config.monitoring as {
        ...
        queueThresholds?: { warning?: number; critical?: number };
        retentionPeriods?: {
          rawData?: number;
      ...
      await upsertConfig(
        ...
        monitoring?.dashboardRefreshSeconds,
        'monitoring'
      );
      await upsertConfig(
        'alert_response_time',
      ...
      await upsertConfig(
        ...
        monitoring?.queueThresholds?.warning,
        'monitoring'
      );
      await upsertConfig(
        'queue_critical_threshold',
        monitoring?.queueThresholds?.critical,
        'monitoring'
      );
