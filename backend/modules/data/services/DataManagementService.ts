...
import DataImportService from './dataManagement/dataImportService';
...
class DataManagementService extends EventEmitter {
  ...
  async getDataList(
    ...
    return this.queryData(type, { filters, search: options.search, userId, workspaceId }, options);
  }

  async getDataRecord(userId: string, id: string, workspaceId?: string) {
    const type = await this.getRecordTypeById(id, { userId, workspaceId });
    if (!type) return null;
    return this.readData(type, id, { userId, workspaceId });
  }
  ...
  async importDataForUpload(
    ...
    const format = (payload.format || 'json') as 'csv' | 'json' | 'excel' | 'xml';
    const importService = this.getImportService();
    const jobId = await importService.createImportTask(
      type,
      payload.file.path,
      {
        format,
  ...
  async getDataVersions(userId: string, id: string, workspaceId?: string) {
    this.ensureIsolationContext({ userId, workspaceId }, '获取版本历史');
    const params: Array<string | number> = [id];
  ...
  async createBackup(
    ...
    try {
      ...
      const backupRecords: Record<string, Array<Record<string, unknown>>> = {};
      ...
      for (const dataType of typesToBackup) {
        ...
        const rowsResult = await query(
          `SELECT id, type, workspace_id, data, metadata, created_at, updated_at
           FROM data_records
           WHERE type = $1 AND deleted_at IS NULL`,
          [dataType]
        );
        backupRecords[dataType] = rowsResult.rows.map(row => ({
          id: row.id,
          type: row.type,
      ...
      const backupDir = path.join(process.cwd(), 'runtime', 'backups');
      await fs.mkdir(backupDir, { recursive: true });

      const jsonFileName = `${backupName}.json`;
      const jsonPath = path.join(backupDir, jsonFileName);
      await fs.writeFile(jsonPath, JSON.stringify(backupPayload, null, 2));

      let outputPath = jsonPath;
      let outputName = jsonFileName;
      ...
      if (format === 'zip') {
        ...
        await fs.unlink(jsonPath).catch((): undefined => undefined);
        outputPath = zipPath;
      ...
      if (options.userId) {
        const expiresInDays =
          typeof options.expiresInDays === 'number' && options.expiresInDays > 0
        ...
        const fileRecord = await storageService.uploadFile(
          ...
          options.userId,
          {
            ownerType: options.workspaceId ? 'workspace' : undefined,
        ...
        fileInfo = {
          ...
          fileName: String(fileRecord.original_name),
      ...
      await fs.unlink(outputPath).catch((): undefined => undefined);
  ...
  async ensureTables() {
    ...
    await query(`CREATE INDEX IF NOT EXISTS idx_data_records_type ON data_records(type)`, []);
    await query(
