...
class DataExportService extends EventEmitter {
  ...
  async cleanupExpiredExports(
    ...
    let deleted = 0;
    for (const row of (tasks.rows || []) as Array<Record<string, string>>) {
      if (row.file_path) {
  ...
  private async processTask(task: ExportTask): Promise<void> {
    ...
    try {
      ...
      if (result.size > this.maxFileSize) {
        throw new Error(
          `导出文件大小 (${(result.size / 1024 / 1024).toFixed(1)}MB) 超过限制 (${(this.maxFileSize / 1024 / 1024).toFixed(1)}MB)`
        );
      }
  ...
  private async encryptFile(
    ...
    // 创建加密器
    const cipher = (
      crypto as unknown as { createCipher: (algo: string, key: string) => unknown }
    ).createCipher(algorithm, password) as NodeJS.ReadWriteStream & {
  ...
  private addPDFTable(
    ...
    const cellWidth = usableWidth / visibleColumns.length;
    const pageBottom = 750;

    const drawRow = (row: Record<string, unknown> | null, y: number, isHeader: boolean): void => {
      const h = isHeader ? headerHeight : cellHeight;
      doc.fontSize(isHeader ? headerFontSize : fontSize);

      // 绘制行背景
      if (isHeader) {
        doc.rect(marginLeft, y, usableWidth, h).fill('#f0f0f0').fillColor('#000000');
      }

      // 绘制单元格文本
      visibleColumns.forEach((column, colIndex) => {
        const x = marginLeft + colIndex * cellWidth + 4;
        const text = isHeader ? column : row ? String(row[column] ?? '') : '';
      ...
      // 绘制水平线
      doc
        .moveTo(marginLeft, y + h)
        .lineTo(marginLeft + usableWidth, y + h)
        .lineWidth(0.5)
        .stroke('#cccccc');
    };
    ...
    // 绘制表头
    let currentY = doc.y;
    drawRow(null, currentY, true);
    currentY += headerHeight;

    // 绘制数据行
    for (const row of data) {
      if (currentY + cellHeight > pageBottom) {
        doc.addPage();
        currentY = 50;
        drawRow(null, currentY, true);
        currentY += headerHeight;
