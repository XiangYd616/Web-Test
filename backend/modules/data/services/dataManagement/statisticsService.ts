...
class StatisticsService {
  ...
  async getTestHistoryStatistics(query: StatisticsQuery): Promise<TestHistoryStatistics> {
    ...
    try {
      ...
      const byDay = (dailyResults.rows || []).map((row: Record<string, unknown>) => ({
    ...
    try {
      ...
      const averageTestsPerUser =
        testsPerUser.rows.length > 0
          ? testsPerUser.rows.reduce(
              (sum: number, row: Record<string, unknown>) => sum + Number(row.test_count || 0),
              0
      ...
      const statistics: UserActivityStatistics = {
        ...
        activityByDay: (dailyActivity.rows || []).map((row: Record<string, unknown>) => ({
          date: String(row.date),
  ...
  async getPerformanceStatistics(query: StatisticsQuery): Promise<PerformanceStatistics> {
    ...
    try {
      ...
      const statistics: PerformanceStatistics = {
        ...
        byEndpoint: (endpointStats.rows || []).map((row: Record<string, unknown>) => ({
  ...
  private async analyzePerformanceTrends(
    ...
    const rows = metricData.rows as Array<Record<string, unknown>> | undefined;
    const metricsRows = rows || [];
    const responseTrend = this.calculateTrend(
      metricsRows.map((row: Record<string, unknown>) => Number(row.response_time) || 0)
    );
    const throughputTrend = this.calculateTrend(
      metricsRows.map((row: Record<string, unknown>) => Number(row.throughput) || 0)
    );
    const errorRateTrend = this.calculateTrend(
      metricsRows.map((row: Record<string, unknown>) => Number(row.error_rate) || 0)
    );
  ...
  private generatePrediction(
    ...
    const ssTotal = values.reduce((s, v) => s + (v - mean) ** 2, 0);
    ...
    for (let i = 1; i <= days; i++) {
      ...
      const predictedValue = trend.slope * (n + i) + trend.intercept;

      // 置信度 = R² 基础值 × 距离衰减因子（越远越不确定）
      const distanceDecay = Math.max(0.3, 1 - i / (n * 2));
      const confidence = Math.round(rSquared * distanceDecay * 100) / 100;
