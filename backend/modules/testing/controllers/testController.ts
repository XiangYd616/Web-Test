...
import { getLogsByTraceId } from '../services/testLogService';
import {
  getDeadLetterJobs,
  getJobDetail,
  getJobsByTraceId,
...
import userTestManager from '../services/UserTestManager';
...
const handleControllerError = (res: ApiResponse, error: unknown, message = '请求处理失败') => {
  ...
  if (errorInfo?.code && errorInfo?.statusCode) {
    return res.error(
      errorInfo.code,
  ...
  return res.error(StandardErrorCode.INTERNAL_SERVER_ERROR, message, errorMessage, 500);
};
...
class TestController {
  ...
  async createAndStart(req: AuthRequest, res: ApiResponse, _next: NextFunction) {
    try {
      const config = req.body as Parameters<typeof testService.createAndStart>[0];
      const testType = String((config as { testType?: string }).testType || '');
      if (!testType) {
        return res.error(
          StandardErrorCode.INVALID_INPUT,
          '测试类型不能为空',
          { code: 'MISSING_TEST_TYPE', details: { availableTypes: TestTypeValues } },
          400
        );
      }
      if (!TestTypeValues.includes(testType as (typeof TestTypeValues)[number])) {
        return res.error(
          StandardErrorCode.TEST_INVALID_TYPE,
          `不支持的测试类型: ${testType}`,
          { code: 'INVALID_TEST_TYPE', details: { availableTypes: TestTypeValues } },
          400
        );
      }
  ...
  async createWebsiteTest(req: AuthRequest, res: ApiResponse, _next: NextFunction) {
    try {
      const config = req.body as Parameters<typeof testService.createWebsiteTest>[0];
      const workspaceId = (config.workspaceId as string | undefined) || undefined;
      if (workspaceId) {
        await ensureWorkspacePermission(workspaceId, req.user.id, 'execute');
      }
  ...
  async createPerformanceTest(req: AuthRequest, res: ApiResponse, _next: NextFunction) {
    try {
      const config = req.body as Parameters<typeof testService.createPerformanceTest>[0];
      const workspaceId = (config.workspaceId as string | undefined) || undefined;
      if (workspaceId) {
        await ensureWorkspacePermission(workspaceId, req.user.id, 'execute');
      }
  ...
  async createSecurityTest(req: AuthRequest, res: ApiResponse, _next: NextFunction) {
    try {
      const config = req.body as Parameters<typeof testService.createSecurityTest>[0];
      const workspaceId = (config.workspaceId as string | undefined) || undefined;
      if (workspaceId) {
        await ensureWorkspacePermission(workspaceId, req.user.id, 'execute');
      }
  ...
  async createSEOTest(req: AuthRequest, res: ApiResponse, _next: NextFunction) {
    try {
      const config = req.body as Parameters<typeof testService.createSEOTest>[0];
      const workspaceId = (config.workspaceId as string | undefined) || undefined;
      if (workspaceId) {
        await ensureWorkspacePermission(workspaceId, req.user.id, 'execute');
      }
  ...
  async createStressTest(req: AuthRequest, res: ApiResponse, _next: NextFunction) {
    try {
      const config = req.body as Parameters<typeof testService.createStressTest>[0];
      const workspaceId = (config.workspaceId as string | undefined) || undefined;
      if (workspaceId) {
        await ensureWorkspacePermission(workspaceId, req.user.id, 'execute');
      }
  ...
  async createAPITest(req: AuthRequest, res: ApiResponse, _next: NextFunction) {
    try {
      const config = req.body as Parameters<typeof testService.createAPITest>[0];
      const workspaceId = (config.workspaceId as string | undefined) || undefined;
      if (workspaceId) {
        await ensureWorkspacePermission(workspaceId, req.user.id, 'execute');
      }
  ...
  async createAccessibilityTest(req: AuthRequest, res: ApiResponse, _next: NextFunction) {
    try {
      const config = req.body as Parameters<typeof testService.createAccessibilityTest>[0];
      const workspaceId = (config.workspaceId as string | undefined) || undefined;
      if (workspaceId) {
        await ensureWorkspacePermission(workspaceId, req.user.id, 'execute');
      }
  ...
  async createCompatibilityTest(req: AuthRequest, res: ApiResponse, _next: NextFunction) {
    try {
      const config = req.body as Parameters<typeof testService.createCompatibilityTest>[0];
      const workspaceId = (config.workspaceId as string | undefined) || undefined;
      if (workspaceId) {
        await ensureWorkspacePermission(workspaceId, req.user.id, 'execute');
      }
  ...
  async createUXTest(req: AuthRequest, res: ApiResponse, _next: NextFunction) {
    try {
      const config = req.body as Parameters<typeof testService.createUXTest>[0];
      const workspaceId = (config.workspaceId as string | undefined) || undefined;
      if (workspaceId) {
        await ensureWorkspacePermission(workspaceId, req.user.id, 'execute');
      }
  ...
  async getProgress(req: AuthRequest, res: ApiResponse, _next: NextFunction) {
    try {
      ...
      // 优先从内存缓存读取实时进度（比数据库更及时）
      const cached = userTestManager.getProgress(testId);
      if (cached) {
        return res.success({
          testId,
