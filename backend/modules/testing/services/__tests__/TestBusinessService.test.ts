  ...
  describe('URL格式验证', () => {
    test('应该接受有效的HTTP URL', async () => {
      ...
      for (const url of validUrls) {
        const result = await service.validateTestConfig(
          { testId: 'test-001', url, testType: 'performance' },
          user
        );
    ...
    test('应该拒绝无效的URL', async () => {
      ...
      for (const url of invalidUrls) {
        const result = await service.validateTestConfig(
          { testId: 'test-002', url: url as string, testType: 'performance' },
          user
        );
    ...
    test('应该拒绝包含恶意脚本的URL', async () => {
      ...
      for (const url of maliciousUrls) {
        const result = await service.validateTestConfig(
          { testId: 'test-003', url, testType: 'performance' },
          user
        );
        expect(result).toEqual(expect.objectContaining({ isValid: false }));
      }
  ...
  describe('并发数验证', () => {
    test('应该接受有效的并发数', async () => {
      ...
      for (const concurrency of validConcurrencies) {
        const result = await service.validateTestConfig(
          { testId: 'test-004', url: 'https://example.com', testType: 'performance', concurrency },
          user
        );
        expect(result).toEqual(expect.objectContaining({ isValid: true }));
      }
    ...
    test('应该拒绝无效的并发数', async () => {
      ...
      for (const concurrency of invalidConcurrencies) {
        const result = await service.validateTestConfig(
          { testId: 'test-005', url: 'https://example.com', testType: 'performance', concurrency },
          user
        );
        expect(result).toEqual(expect.objectContaining({ isValid: false }));
      }
  ...
  describe('测试时长验证', () => {
    test('应该接受有效的测试时长', async () => {
      ...
      for (const duration of validDurations) {
        const result = await service.validateTestConfig(
          { testId: 'test-006', url: 'https://example.com', testType: 'performance', duration },
          user
        );
        expect(result).toEqual(expect.objectContaining({ isValid: true }));
      }
    ...
    test('应该拒绝无效的测试时长', async () => {
      ...
      for (const duration of invalidDurations) {
        const result = await service.validateTestConfig(
          { testId: 'test-007', url: 'https://example.com', testType: 'performance', duration },
          user
        );
        expect(result).toEqual(expect.objectContaining({ isValid: false }));
      }
...
describe('TestBusinessService - 业务规则验证', () => {
  ...
  describe('权限控制', () => {
    test('应该允许有权限的用户创建测试', async () => {
      ...
      await expect(
        service.validateTestConfig({ ...config, testId: 'test-008' }, user)
    ...
    test('应该拒绝没有权限的用户创建测试', async () => {
      ...
      await expect(
        service.validateTestConfig({ ...config, testId: 'test-009' }, user)
      ).resolves.toEqual(expect.objectContaining({ isValid: false }));
    });
    ...
    test('应该处理没有权限记录的用户', async () => {
      ...
      await expect(
        service.validateTestConfig({ ...config, testId: 'test-010' }, user)
      ).resolves.toEqual(expect.objectContaining({ isValid: true }));
    });
  });
});
...
describe('TestBusinessService - 配置校验逻辑', () => {
  ...
  test('应该返回验证结果', async () => {
    ...
    const result = await service.validateTestConfig(
      { testId: 'test-008', url: 'https://example.com', testType: 'performance' },
      user
    );
