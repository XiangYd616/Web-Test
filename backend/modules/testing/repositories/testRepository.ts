...
class TestRepository {
  ...
  async findById(
    ...
    const clause = this.buildScopedClause(params, userId, workspaceId);
    const result = await query(
      `SELECT * FROM test_executions WHERE test_id = $1 AND ${clause}`,
      params
    );
  ...
  async checkOwnership(testId: string, userId: string, workspaceId?: string): Promise<boolean> {
    ...
    const clause = this.buildScopedClause(params, userId, workspaceId);
    const result = await query(
      `SELECT 1 FROM test_executions WHERE test_id = $1 AND ${clause}`,
      params
    );
