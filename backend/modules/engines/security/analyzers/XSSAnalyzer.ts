...
class XSSAnalyzer {
  ...
  async analyze(
    ...
    try {
      ...
      // Puppeteer 可用性检查：反射型/DOM型/存储型 XSS 测试均依赖浏览器
      ...
      if (poolAvailable) {
        ...
        browserSkipped = true;
        console.warn(
          '[XSSAnalyzer] Puppeteer 不可用，跳过浏览器级 XSS 检测（反射型/DOM型/存储型）'
        );
      }
  ...
  > {
    // 不再盲目测试硬编码端点（大多数返回 404，浪费时间且可能触发 WAF）
    // 仅对目标 URL 本身添加常见参数名进行探测
    const urlObj = new URL(url);
    ...
    return [
      {
        ...
        method: 'GET',
        parameters: [
          { name: 'id', type: 'query' as const, value: '1' },
          { name: 'search', type: 'query' as const, value: 'test' },
        ],
      },
    ];
  }
  ...
  private async testReflectedXSS(
    ...
    const { page, release } = await puppeteerPool.acquirePage({
      width: browserOptions.viewport.width,
      height: browserOptions.viewport.height,
      warmupUrl: testPoint.url,
      headed: !browserOptions.headless,
    });

    try {
      // 只注册一次 XSS 监听器（避免 evaluateOnNewDocument 在复用页面上累积）
      await page.evaluateOnNewDocument(() => {
        const xssWindow = window as unknown as {
          xssDetected: boolean;
          xssPayload: string;
          __xssListenerInstalled?: boolean;
        };
        if (xssWindow.__xssListenerInstalled) return;
        xssWindow.__xssListenerInstalled = true;
        xssWindow.xssDetected = false;
        xssWindow.xssPayload = '';

        const originalAlert = window.alert;
        window.alert = function (message) {
          const w = window as unknown as { xssDetected: boolean; xssPayload: string };
          w.xssDetected = true;
          w.xssPayload = message;
  ...
  private async testDOMBasedXSS(
    ...
    const vulnerabilities: XSSVulnerability[] = [];
    const { page: domPage, release: domRelease } = await puppeteerPool.acquirePage({
      width: browserOptions.viewport.width,
      height: browserOptions.viewport.height,
      warmupUrl: url,
      headed: !browserOptions.headless,
    ...
    try {
      // 设置XSS监听器
      await domPage.evaluateOnNewDocument(() => {
        const xssWindow = window as unknown as {
          xssDetected: boolean;
          xssPayload: string;
        };
      ...
      for (const payload of this.payloads.dom) {
        try {
          ...
          await domPage.goto(testUrl, {
            waitUntil: 'networkidle0',
            timeout: browserOptions.timeout,
          });

          // 检查XSS是否触发
          const xssDetected = await domPage.evaluate(() => {
            const xssWindow = window as unknown as {
              xssDetected?: boolean;
            };
            return xssWindow.xssDetected ?? false;
          });
          const xssPayload = await domPage.evaluate(() => {
            const xssWindow = window as unknown as {
              xssPayload?: string;
            };
  ...
  private async testStoredXSS(
    ...
    const vulnerabilities: XSSVulnerability[] = [];
    const { page: storedPage, release: storedRelease } = await puppeteerPool.acquirePage({
      width: browserOptions.viewport.width,
      height: browserOptions.viewport.height,
      warmupUrl: _url,
      headed: !browserOptions.headless,
    ...
    try {
      // 设置XSS监听器
      await storedPage.evaluateOnNewDocument(() => {
        const xssWindow = window as unknown as {
          xssDetected: boolean;
          xssPayload: string;
        };
      ...
      for (const form of forms) {
        for (const payload of this.payloads.basic.slice(0, 3)) {
          ...
          try {
            ...
            await storedPage.goto(form.url, {
              waitUntil: 'networkidle0',
              timeout: browserOptions.timeout,
            });

            // 填写表单
            await storedPage.evaluate((data: Record<string, string>) => {
            ...
            // 提交表单（先检查 submit 按钮是否存在）
            const hasSubmit = await storedPage.$('button[type="submit"], input[type="submit"]');
            ...
            await Promise.race([
              Promise.all([
                storedPage
                  ...
                  .catch(() => {}),
                storedPage.click('button[type="submit"], input[type="submit"]'),
              ]),
              new Promise(resolve => setTimeout(resolve, browserOptions.timeout)),
            ]);

            // 检查XSS是否触发
            const xssDetected = await storedPage.evaluate(() => {
              const xssWindow = window as unknown as {
                xssDetected?: boolean;
              };
              return xssWindow.xssDetected ?? false;
            });
            const xssPayload = await storedPage.evaluate(() => {
              const xssWindow = window as unknown as {
                xssPayload?: string;
              };
              return xssWindow.xssPayload ?? '';
  ...
  private async testBasicXSS(
    ...
    // XSS 监听器已在 testReflectedXSS 中通过 evaluateOnNewDocument 注册一次
    // 此处不再重复注册，避免在复用页面上累积多个拦截器
