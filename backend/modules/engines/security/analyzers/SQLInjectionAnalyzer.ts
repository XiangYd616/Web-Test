...
class SQLInjectionAnalyzer {
  ...
  constructor() {
    ...
    this.payloads = {
      ...
      // 基于时间的盲注（使用 2s 延迟减少对目标数据库的影响，每种 DB 只保留 1 个 payload）
      timeBased: [
        "'; WAITFOR DELAY '00:00:02'--",
        "' AND (SELECT SLEEP(2))--",
        "' AND (SELECT pg_sleep(2))--",
        "' AND (SELECT sqlite3_sleep(2000))--",
      ],
    ...
    for (const payload of this.payloads.timeBased) {
      try {
        ...
        if (responseTime > 1800) {
          // 超过1.8秒认为存在时间注入（payload 使用 2s 延迟）
          vulnerabilities.push({
            type: 'time-based',
            payload,
            parameter: parameter.name,
  ...
  private isSQLInjectionResponse(response: AxiosResponse<unknown>, payload: string): boolean {
    ...
    // 检查UNION注入成功特征：响应中包含数据库版本/元数据信息才算成功
    if (payload.includes('UNION')) {
      const unionSuccessPatterns = [
        /\d+\.\d+\.\d+/, // 版本号模式 (如 5.7.38, 14.2.0)
        /information_schema/i,
        /mysql|mariadb|postgresql|sqlite|oracle|mssql/i,
        /root@localhost/i,
      ];
      if (unionSuccessPatterns.some(p => p.test(responseText))) {
        return true;
      }
    }

    // 注意：移除了旧的 --/# 注释绕过检测，因为它会对任何非空响应误报
    // 注释绕过的真正检测应通过盲注差异分析完成（见 isBlindSQLInjection）
  ...
  private isBlindSQLInjection(
    ...
    if (trueResponse.status !== falseResponse.status) {
      // 一个成功一个失败（如 200 vs 500）是明确的盲注信号
      const trueOk = trueResponse.status >= 200 && trueResponse.status < 400;
      const falseOk = falseResponse.status >= 200 && falseResponse.status < 400;
      if (trueOk !== falseOk) {
        return true;
      }
    }
    ...
    if (trueText !== falseText) {
      ...
      const maxLength = Math.max(trueLength, falseLength, 1);

      // 需要同时满足：绝对差异 > 500 字节 且 相对差异 > 20%
      // 这样可以过滤掉动态内容（时间戳、nonce、广告等）的微小变化
      if (lengthDiff > 500 && lengthDiff / maxLength > 0.2) {
        return true;
      }
    }
