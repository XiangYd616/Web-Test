...
class PerformanceAnalyzer {
  ...
  private async collectPerformanceData(
    ...
    const data = await page.evaluate(() => {
      ...
      const navTiming = navEntry as unknown as
        | {
            startTime: number;
  ...
  private isOptimizedCaching(resources: ResourceInfo[]): boolean {
    ...
    if (!cacheableResources.length) {
      return false;
    }
    const cachedResources = cacheableResources.filter(r => r.cached);
    const compressedResources = cacheableResources.filter(r => r.compressed);
    const cacheHitRate = cachedResources.length / cacheableResources.length;
    const compressionRate = compressedResources.length / cacheableResources.length;
    return cacheHitRate >= 0.8 && compressionRate >= 0.6;
  }
