...
export class EnhancedBaseService extends BaseService {
  ...
  private async checkDependenciesHealth(): Promise<HealthCheckResult['checks'][0]> {
    ...
    try {
      ...
      const timeoutMs = this.healthCheckConfig.timeout || 5000;
      ...
      const status: 'pass' | 'warn' | 'fail' =
        failedRequired.length > 0 ? 'fail' : failedOptional.length > 0 ? 'warn' : 'pass';
      const messageParts = results.map(
        result =>
          `${result.name}:${result.ok ? 'ok' : result.required ? 'failed' : 'optional_failed'}`
      );
      return {
        name: 'dependencies',
        status,
