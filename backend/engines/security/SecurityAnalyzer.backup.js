...
class SecurityAnalyzer {
  ...
  async analyze(url, config = {}) {
    ...
    try {
      ...
      try {
        // 使用SSL分析器进行深度分析
        const advancedSSLResults = await this.sslAnalyzer.analyze(url);
      ...
      try {
        // 使用安全头分析器进行深度分析
        const advancedHeaderResults = await this.securityHeadersAnalyzer.analyze(url);
