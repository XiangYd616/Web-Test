...
interface SecurityHeaders {
  ...
  'cross-origin-embedder-policy': boolean;
  'cross-origin-opener-policy': boolean;
  'cross-origin-resource-policy': boolean;
}
...
class SecurityTestCore {
  ...
  async runSecurityTest(url: string, config: SecurityTestConfig = {}): Promise<SecurityTestResult> {
    ...
    try {
      ...
      // 并行执行各项安全测试
      const [sslResult, headersResult, vulnerabilityResult, configurationResult] = await Promise.all([
   ...
   * 检查SSL/TLS配置
   */
  private async checkSSL(url: string, config: SecurityTestConfig): Promise<{
    score: number;
    enabled: boolean;
    valid: boolean;
    protocol: string;
    ...
    try {
      ...
      const port = parsedUrl.port || (parsedUrl.protocol === 'https:' ? 443 : 443);
   ...
   * 检查安全头
   */
  private async checkSecurityHeaders(url: string, config: SecurityTestConfig): Promise<{
    score: number;
    present: Record<string, boolean>;
    missing: string[];
    issues: string[];
   ...
   * 验证安全头值
   */
  private validateSecurityHeader(header: string, value: string): { valid: boolean; issue?: string } {
    switch (header) {
      case 'strict-transport-security':
        return {
          valid: value.includes('max-age') && (value.includes('includeSubDomains') || value.includes('preload')),
      ...
      case 'x-frame-options':
        return {
          valid: ['DENY', 'SAMEORIGIN', 'ALLOW-FROM'].some(option => value.toUpperCase().includes(option)),
   ...
   * 扫描漏洞
   */
  private async scanVulnerabilities(url: string, config: SecurityTestConfig): Promise<{
    score: number;
    xss: boolean;
    sqlInjection: boolean;
    csrf: boolean;
    ...
    // XSS检测
    const xssResult = await this.detectXSS(url);
    ...
    // SQL注入检测
    const sqlResult = await this.detectSQLInjection(url);
   ...
   * 检测XSS漏洞
   */
  private async detectXSS(url: string): Promise<{ vulnerable: boolean; issues: Array<{ type: string; severity: string; description: string }> }> {
    // 简化的XSS检测逻辑
   ...
   * 检测SQL注入漏洞
   */
  private async detectSQLInjection(url: string): Promise<{ vulnerable: boolean; issues: Array<{ type: string; severity: string; description: string }> }> {
    const issues: Array<{ type: string; severity: string; description: string }> = [];
   ...
   * 检测CSRF漏洞
   */
  private async detectCSRF(url: string): Promise<{ vulnerable: boolean; issues: Array<{ type: string; severity: string; description: string }> }> {
    const issues: Array<{ type: string; severity: string; description: string }> = [];
   ...
   * 检测目录遍历漏洞
   */
  private async detectDirectoryTraversal(url: string): Promise<{ vulnerable: boolean; issues: Array<{ type: string; severity: string; description: string }> }> {
    const issues: Array<{ type: string; severity: string; description: string }> = [];
   ...
   * 检测开放重定向漏洞
   */
  private async detectOpenRedirect(url: string): Promise<{ vulnerable: boolean; issues: Array<{ type: string; severity: string; description: string }> }> {
    const issues: Array<{ type: string; severity: string; description: string }> = [];
   ...
   * 检查配置
   */
  private async checkConfiguration(url: string, config: SecurityTestConfig): Promise<{
    score: number;
    methods: string[];
    cors: {
      enabled: boolean;
  ...
  private generateRecommendations(results: {
    ...
    // 漏洞建议
    results.vulnerabilities.issues.forEach((issue: any) => {
      recommendations.push({
