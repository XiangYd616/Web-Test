...
class JwtService {
  ...
  constructor() {
    ...
    this.accessTokenSecret =
      process.env.JWT_SECRET || 'testweb-super-secret-jwt-key-change-in-production-2024';
    this.refreshTokenSecret =
      process.env.JWT_REFRESH_SECRET || 'testweb-refresh-secret-key-change-in-production-2024';
   ...
   * @returns {string} 访问令牌
   */
  generateAccessToken(payload: Record<string, unknown>, expiresIn: string | null = null) {
   ...
   * @returns {string} 刷新令牌
   */
  generateRefreshToken(payload: Record<string, unknown>, expiresIn: string | null = null) {
    const tokenPayload: TokenPayload = {
      ...(payload as TokenPayload),
      type: 'refresh',
   ...
   * @param {string} refreshToken - 刷新令牌
   */
  async storeRefreshToken(userId: number | string, refreshToken: string) {
    try {
      const decoded = jwt.decode(refreshToken);
      if (!decoded || typeof decoded === 'string' || !(decoded as JwtPayload).exp) {
   ...
   */
  async cleanupExpiredTokens(userId: number | string | null = null) {
    try {
   ...
   */
  async getUserActiveTokenCount(userId: number | string) {
    try {
   ...
   * @returns {number} 秒数
   */
  parseExpiryToSeconds(expiry: string) {
    ...
    return Number.parseInt(value, 10) * (units[unit] || 1);
  }
   ...
   */
  isTokenExpiringSoon(token: string, thresholdMinutes = 5) {
    try {
