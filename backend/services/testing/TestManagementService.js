...
const EnhancedWebSocketManager = require('../streaming/WebSocketManager');
...
class TestManagementService extends EventEmitter {
    ...
    engineConfigs.forEach(config => {
      try {
        ...
        this.engines.set(config.id, {
          ...
          instance: engine,
          status: 'idle',
          supportsEvents: typeof engine.on === 'function',
          metrics: {
            totalTests: 0,
            successRate: 100,
            avgExecutionTime: 0,
  ...
  async updateTestStatus(testId, status, progress = null) {
    let sql = `UPDATE test_history SET status = $2`;
    const params = [testId, status];
    let paramIndex = 3;

    if (progress !== null) {
      sql += `, progress = $${paramIndex}`;
