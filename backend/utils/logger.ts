...
// 日志级别配置
const LOG_LEVELS: LogLevelConfig = {
  ERROR: 0,
  WARN: 1,
  INFO: 2,
  DEBUG: 3,
 ...
 * 格式化日志消息
 */
function formatLogMessage(level: string, message: string, meta: LogMeta = {}) {
...
class Logger {
   ...
   * 错误日志
   */
  static error(message: string, error: Error | unknown = null, meta: LogMeta = {}) {
   ...
   * 数据库操作日志
   */
  static db(operation: string, table: string, duration: number, meta: LogMeta = {}) {
   ...
   * 测试相关日志
   */
  static test(testId: string, phase: string, message: string, meta: LogMeta = {}) {
   ...
   * 性能监控日志
   */
  static perf(operation: string, duration: number, meta: LogMeta = {}) {
   ...
   * 安全相关日志
   */
  static security(event: string, details: string, meta: LogMeta = {}) {
   ...
   * 用户操作日志
   */
  static user(userId: string, action: string, details: string, meta: LogMeta = {}) {
   ...
   * 系统事件日志
   */
  static system(event: string, details: string, meta: LogMeta = {}) {
