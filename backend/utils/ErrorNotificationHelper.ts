...
type RealtimeService = {
  notifyTestFailed?: (testId: string, errorInfo: Record<string, unknown>) => Promise<void> | void;
  notifyTestWarning?: (
    testId: string,
    warningInfo: Record<string, unknown>
  ) => Promise<void> | void;
  notifyTestTimeout?: (
    testId: string,
    timeoutInfo: Record<string, unknown>
  ) => Promise<void> | void;
};

declare global {
  var realtimeService: RealtimeService | undefined;
}
...
class ErrorNotificationHelper {
   ...
   * 发送详细的测试失败通知
   */
  async sendTestFailedNotification(testId: string, error: ErrorLike, context: ErrorContext = {}) {
    try {
      const errorInfo = {
        testId,
   ...
   * 发送测试超时通知
   */
  async sendTestTimeoutNotification(
    testId: string,
    timeoutInfo: { duration?: number; limit?: number; stage?: string },
    context: ErrorContext = {}
  ) {
    try {
      const timeoutNotification = {
        ...
        retryable: true,
        severity: 'medium',
        suggestions: ['检查网络连接', '增加超时时间限制', '稍后重试测试'],
      };
   ...
   * 判断错误是否可重试
   */
  isRetryableError(error: ErrorLike) {
   ...
   * 获取错误严重程度
   */
  getErrorSeverity(error: ErrorLike) {
   ...
   * 获取错误分类
   */
  getErrorCategory(error: ErrorLike) {
    const code = error.code || '';
    const message = (error.message || '').toLowerCase();
   ...
   * 获取错误建议
   */
  getErrorSuggestions(error: ErrorLike) {
    const category = this.getErrorCategory(error);

    const suggestions: Record<string, string[]> = {
      network: ['检查网络连接', '验证URL是否可访问', '检查防火墙设置', '稍后重试'],
      timeout: ['增加超时时间限制', '检查网络速度', '分批处理大型页面', '稍后重试'],
      validation: ['检查输入参数格式', '验证URL有效性', '确认配置参数正确', '查看详细错误信息'],
      authentication: ['检查API密钥', '验证访问权限', '更新认证信息', '联系管理员'],
      analysis: ['检查页面内容', '验证页面结构', '确认页面可访问性', '尝试不同的分析配置'],
      general: ['查看详细错误日志', '检查系统状态', '稍后重试', '联系技术支持'],
    };
   ...
   * 获取警告严重程度
   */
  getWarningSeverity(warning: WarningLike) {
   ...
   * 获取警告建议
   */
  getWarningSuggestions(warning: WarningLike) {
    const code = warning.code || '';
    const message = (warning.message || '').toLowerCase();
   ...
   * 清理配置对象（移除敏感信息）
   */
  sanitizeConfig(config: Record<string, unknown>) {
...
export default ErrorNotificationHelper;
