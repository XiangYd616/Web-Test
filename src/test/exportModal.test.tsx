...
describe('ExportModal', () => {
  ...
  it('导出过程中应该显示加载状态', async () => {
    const mockOnExport = jest.fn().mockImplementation(() => new Promise(resolve => setTimeout(resolve, 100)));
    render(<ExportModal {...defaultProps} onExport={mockOnExport} />);
  ...
  it('导出失败时应该显示错误提示', async () => {
    const mockOnExport = jest.fn().mockRejectedValue(new Error('导出失败'));
