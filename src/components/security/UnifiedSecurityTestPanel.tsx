...
export const UnifiedSecurityTestPanel = forwardRef<UnifiedSecurityTestPanelRef, UnifiedSecurityTestPanelProps>(({
  onTestStart,
  onTestProgress,
  onTestComplete,
  onTestError
  ...
  const runTest = useCallback(async () => {
    ...
    try {
      ...
      // 创建增强错误
      const enhancedErr = enhanceError(err instanceof Error ? err : new Error(errorMessage), {
        url: config.url,
  ...
  return (
    <div className="unified-security-test-panel space-y-4 sm:space-y-6 fade-in-up">
      {/* URL 输入 - 增强版 */}
      <div className="bg-gray-800/90 backdrop-blur-sm rounded-xl border border-gray-700/60 p-5 sm:p-7 shadow-lg">
...
UnifiedSecurityTestPanel.displayName = 'UnifiedSecurityTestPanel';
