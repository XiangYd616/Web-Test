...
import { ComponentType, lazy    } from 'react';// 预加载状态管理'
...
class RoutePreloader {
  ...
  async preloadRoutes(routes: Array<{ path: string; importFn: () => Promise<any> }>): Promise<void> {
    const promises = routes.map(({ path, importFn }) => this.preloadRoute(path, importFn).catch(() => { }) // 忽略单个路由的错误
    );
 ...
 * 创建带预加载的懒加载组件
 */
export const createLazyComponent = <T extends ComponentType<any>>(importFn: () => Promise<{ default: T }>,
  routePath?: string
) => {
