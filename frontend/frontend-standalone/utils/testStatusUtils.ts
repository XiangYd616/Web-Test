...
// 获取状态文本
export const _getStatusText = (status: string): string => {
...
// 获取状态样式类
export const _getStatusStyleClasses = (status: string): string => {
  const config = getStatusConfig(status);
  const isAnimated = status === 'running';
...
// 计算测试完成度
export const _calculateTestCompletion = (record: unknown): number => {
  if (!record) return 0;
