...
export type AppAction =
  ...
  // 测试相关
  | { type: 'TEST_START'; payload: { test: TestState['activeTests'][0] } }
  | { type: 'TEST_UPDATE_PROGRESS'; payload: { testId: string; progress: number } }
  | { type: 'TEST_COMPLETE'; payload: { testId: string; result: TestState['history'][0] } }
  | { type: 'TEST_FAIL'; payload: { testId: string; error: string } }
  | { type: 'TEST_CANCEL'; payload: { testId: string } }
  | { type: 'TEST_ADD_TO_HISTORY'; payload: { result: TestState['history'][0] } }
  | { type: 'TEST_SAVE_CONFIGURATION'; payload: { config: TestState['configurations'][0] } }
  | { type: 'TEST_CLEAR_ERROR' }

  // 监控相关
  | { type: 'MONITORING_START' }
  | { type: 'MONITORING_STOP' }
  | { type: 'MONITORING_ADD_TARGET'; payload: { target: MonitoringState['targets'][0] } }
  | {
      type: 'MONITORING_UPDATE_TARGET';
      payload: { targetId: string; updates: Partial<MonitoringState['targets'][0]> };
    }
  | { type: 'MONITORING_REMOVE_TARGET'; payload: { targetId: string } }
  | { type: 'MONITORING_ADD_ALERT'; payload: { alert: MonitoringState['alerts'][0] } }
  ...
  | { type: 'UI_ADD_NOTIFICATION'; payload: { notification: UIState['notifications'][0] } }
      ...
      return {
        ...
        monitoring: {
          ...
          alerts: state.monitoring.alerts.map(alert =>
            alert.id === action.payload.alertId ? { ...alert, resolved: true } : alert
          ),
        },
      };
