...
import { ProxyResponse, proxyService  } from '../integration/proxyService';// SEO分析结果接口'
...
💡 操作提示：点击下方'立即使用本地分析'按钮，上传您的HTML文件即可开始分析。`);'`
  ...
  private checkCanonicalTags(dom: Document, currentUrl: string): {
    ...
    if (!canonicalUrl) {
      ...
      return {
        present: true, correct: false, issues
      };
    }
  ...
  private checkColorContrast(dom: Document): {
    ...
    textElements.forEach((element, index) => {
      ...
      // 检查内联样式中的颜色问题（简化版本）
      const hasLightText = /color\s*:\s*(#[fF]{3,6}|white|#fff)/i.test(style);
      const hasLightBackground = /background(-color)?\s*:\s*(#[fF]{3,6}|white|#fff)/i.test(style);
      const hasDarkText = /color\s*:\s*(#[0-9a-fA-F]{0,2}|black|#000)/i.test(style);
      const hasDarkBackground = /background(-color)?\s*:\s*(#[0-9a-fA-F]{0,2}|black|#000)/i.test(style);
  ...
  private checkViewport(dom: Document): {
    ...
    if (!viewportMeta) {
      ...
      return {
        present: false, correct: false, content: '';
      };
    }
