...
import { AlertCircle, AlertTriangle, BarChart3, CheckCircle, Clock, Download, FileText, Globe, Loader, Play, RotateCcw, Settings, Shield, Square, TrendingUp, Users, XCircle, Zap    } from 'lucide-react';import React, { useCallback, useEffect, useRef, useState    } from 'react';import { useLocation    } from 'react-router-dom';import { useAuthCheck    } from '../../../components/auth/WithAuthCheck.tsx';import { StressTestCharts    } from '../../../components/charts/index';import StressTestHistory from '../../../components/testing/StressTestHistory.tsx';import URLInput from '../../../components/testing/URLInput';import CancelProgressFeedback from '../../../components/ui/CancelProgressFeedback.tsx';import CancelTestConfirmDialog from '../../../components/ui/CancelTestConfirmDialog.tsx';import ExportModal from '../../../components/ui/ExportModal.tsx';import { useLocalStressTest    } from '../../../hooks/useLocalStressTest.ts';import { StressTestConfig as ImportedAdvancedStressTestConfig    } from '../../../hooks/useSimpleTestEngine.ts';import { useStressTestRecord    } from '../../../hooks/useStressTestRecord.ts';import { useUserStats    } from '../../../hooks/useUserStats.ts';import backgroundTestManager from '../../../services/testing/backgroundTestManager.ts';import ExportUtils from '../../../utils/exportUtils.ts';import { systemResourceMonitor    } from '../../../services/system/systemResourceMonitor.ts';import { testEngineManager    } from '../../../services/testing/testEngines.ts';import { TestPhase    } from '../../../services/testing/testStateManager';// import { getTemplateById   } from '../../../services/testTemplates.ts';// 函数不存在，已注释'
import '../../../styles/progress-bar.css';
import type { TestStatusType  } from '../../../types/testHistory.ts';// 工具函数：安全地从URL获取主机名'
    ...
    useEffect(() => {
        ...
        const testStartTime = stressTestData.length > 0 ?
            (typeof stressTestData[0].timestamp === 'string' ?'
                new Date(stressTestData[0].timestamp).getTime(): stressTestData[0].timestamp) :
            Date.now();
    ...
    return (
        <div className= 'space-y-4 dark-page-scrollbar'>
            <div className= 'space-y-6'>
                    ...
                    <div className= 'relative p-6'>
                        <div className= 'flex flex-col lg:flex-row lg:items-center justify-between gap-4'>
                            ...
                            <div className= 'flex items-center space-x-2'>
                                {activeTab === 'test' && (<div className= 'flex items-center bg-gray-700/50 rounded-md p-0.5'>
                                        <button
                                            type= 'button';
                                ...
                                <div className= "flex items-center space-x-2'>`
                                    ...
                                    ) : testStatus === 'failed' ? (<div className= 'flex items-center space-x-2'>
                ...
                {/* 根据标签页显示不同内容 */}
                {
                    activeTab === 'test' ? (<>
                            ...
                            <div className= 'bg-gray-800/80 backdrop-blur-sm rounded-lg border border-gray-700/50 p-4 url-input-card'>
                                ...
                                {(testProgress || backgroundTestInfo || testStatus !== 'idle' || result || metrics) && (<div className= 'mt-4 pt-4 border-t border-gray-700/50'>
                                        {(() => {
                                    ...
                                    <div className= 'xl:col-span-2 bg-gray-800/80 backdrop-blur-sm rounded-xl border border-gray-700/50 p-6'>
                                        ...
                                        {localStressTest.isAvailable && (
                                            <div className= 'mb-4 p-4 bg-gradient-to-r from-purple-500/10 to-blue-500/10 border border-purple-500/20 rounded-lg'>
                                                ...
                                                {useLocalTest && (<div className= 'grid grid-cols-2 gap-3 text-xs'>
                                        ...
                                        <div className= 'mt-4 bg-gray-800/80 backdrop-blur-sm rounded-xl border-2 border-blue-500/30 p-4'>
                                            ...
                                            <div className= 'space-y-3'>
                                                ...
                                                {/* 代理配置 */}
                                                {testConfig.proxy?.enabled && (<div className= 'space-y-2 pl-3 border-l-2 border-blue-500/30'>
                                                        {/* 代理类型 */}
                                                        <div>
                                                        ...
                                                        <div className= 'bg-gray-700/30 rounded-lg border border-gray-600/50'>
                                                            ...
                                                            {showQuickProxySettings && (<div className= 'px-3 pb-3'>
                                                        ...
                                                        <div className= 'bg-blue-500/10 border border-blue-500/30 rounded-lg p-3'>
                                                            ...
                                                            {(proxyTestStatus.result || proxyTestStatus.testing) && (
                                                                <div className={`flex items-center justify-between text-xs p-2 rounded ${proxyTestStatus.result === 'success';'`}
                                                                    ...
                                                                    {/* 关闭按钮 - 只在成功状态显示 */}
                                                                    {proxyTestStatus.result === 'success' && (<button'
                                                                            type= 'button';
                            ...
                            {/* 测试结果 */}
                            {(result || metrics) && (<div className= 'bg-gray-800/80 backdrop-blur-sm rounded-xl border border-gray-700/50 p-4'>
                                    ...
                                    {/* 数据传输分析 */}
                                    {(result?.metrics?.dataReceived || metrics?.dataReceived || result?.metrics?.dataSent || metrics?.dataSent) && (<div className= 'bg-gray-700/50 rounded-lg p-4 mb-6'>
                                            <h4 className= 'text-lg font-semibold text-white mb-4 flex items-center'>
                            ...
                            {/* 统一压力测试图表 - 空间复用 */}
                            {useUnifiedCharts ? (<div className= 'bg-gray-800/80 backdrop-blur-sm rounded-xl border border-gray-700/50 p-6'>
                                    <div className= 'flex items-center justify-between mb-4'>
                                    ...
                                    <div className= 'bg-gray-800/80 backdrop-blur-sm rounded-xl border border-gray-700/50 p-6'>
                                        ...
                                        ) : stressTestData && stressTestData.length > 0 ? (/* 显示测试完成后的数据 */
                ...
                {/* 取消进度反馈 */}
                <CancelProgressFeedback isVisible={showCancelProgress}
            ...
            {/* 历史标签页内容 */}
            {
                activeTab === "history' && ('')`
                    <div className= 'mt-6'>
                        <StressTestHistory onTestSelect={handleTestSelect}
