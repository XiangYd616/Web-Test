...
const APITest: React.FC = () => {
  ...
  const { progress: apiProgress } = useTestProgress(currentTestId || undefined, {
    ...
    onComplete: result => {
      Logger.debug('✅ 基础设施测试完成:', result);
      setResult(result);
      setTestStatus('completed');
      recordTestCompletion('基础设施测试', true, result?.score || 0, result?.duration || 30);
    },
    onError: error => {
        ...
        switch (event) {
          ...
          case 'testCompleted':
            ...
            if (testInfo.result) {
              ...
              recordTestCompletion('基础设施测试', success, score, duration);
            }
            break;
          case 'testFailed':
            setBackgroundTestInfo(testInfo);
  ...
  const handleExportReport = (format: 'json' | 'csv' | 'html') => {
    ...
    switch (format) {
      case 'json': {
        ...
        jsonLink.download = `infrastructure-test-report-${Date.now()}.json`;
      ...
      case 'csv': {
        ...
        csvLink.href = csvUrl;
        csvLink.download = `infrastructure-test-report-${Date.now()}.csv`;
      ...
      case 'html': {
        ...
        htmlLink.download = `infrastructure-test-report-${Date.now()}.html`;
