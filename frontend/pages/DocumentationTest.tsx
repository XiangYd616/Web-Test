...
const APITest: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    'basic' | 'auth' | 'headers' | 'environment' | 'advanced'
  >('basic');
  ...
  const {
    ...
    onProgress: progressData => {
      Logger.debug('📊 文档生成进度:', progressData);
      setTestProgress(progressData.message);
    },
    onComplete: result => {
      Logger.debug('✅ 文档生成完成:', result);
      setResult(result);
      setTestStatus('completed');
      recordTestCompletion('文档生成', true, result?.score || 0, result?.duration || 30);
    },
    onError: error => {
        ...
        switch (event) {
          ...
          case 'testCompleted':
            ...
            if (testInfo.result) {
              ...
              recordTestCompletion('文档生成', success, score, duration);
            }
            break;
          case 'testFailed':
            setBackgroundTestInfo(testInfo);
  ...
  const handleStartTest = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        throw new Error(response.message || '启动文档生成失败');
      }
  ...
  const updateEndpoint = (id: string, updates: Partial<APIEndpoint>) => {
    setTestConfig(prev => ({
      ...prev,
      endpoints: prev.endpoints.map(ep => (ep.id === id ? { ...ep, ...updates } : ep)),
    }));
  };
  ...
  const handleExportReport = (format: 'json' | 'csv' | 'html') => {
    ...
    switch (format) {
      case 'json': {
        ...
        jsonLink.download = `documentation-test-report-${Date.now()}.json`;
      ...
      case 'csv': {
        const csvData = [
          ...
          ...(result?.endpointResults || []).map((ep: any) => [
            ...
            ep.method,
            ep.status,
            (Math.round(ep.responseTime * 100) / 100).toFixed(2),
            ep.statusCode,
            (ep.errors || []).length,
          ]),
        ];
        ...
        csvLink.href = csvUrl;
        csvLink.download = `documentation-test-report-${Date.now()}.csv`;
      ...
      case 'html': {
        ...
        htmlLink.download = `documentation-test-report-${Date.now()}.html`;
                  ...
                  {backgroundTestInfo && backgroundTestInfo.startTime && (
                    <div className="flex items-center space-x-2 mt-2 text-xs text-blue-200">
                      ...
                      <span>
                        运行时长:{' '}
                        {Math.floor(
                          (Date.now() - new Date(backgroundTestInfo.startTime).getTime()) / 1000
                        )}
                        秒
                      </span>
                    </div>
              ...
              <div className="bg-gray-800/80 backdrop-blur-sm rounded-xl border border-gray-700/50 p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div
                    ...
                    onClick={() =>
                      setTestConfig(prev => ({ ...prev, testSecurity: !prev.testSecurity }))
                    }
                  >
                    <div className="flex items-center space-x-3 mb-3">
                      <div
                        className={`p-2 rounded-lg ${testConfig.testSecurity ? 'bg-red-500/20' : 'bg-gray-600/20'}`}
                      >
                        <Shield
                          className={`w-5 h-5 ${testConfig.testSecurity ? 'text-red-400' : 'text-gray-400'}`}
                        />
                      </div>
                      <div className="flex-1">
                        <h4 className="font-semibold text-white">
                          安全测试 <span className="text-xs text-yellow-400">(可选)</span>
                        </h4>
                  ...
                  <div
                    ...
                    onClick={() =>
                      setTestConfig(prev => ({ ...prev, testPerformance: !prev.testPerformance }))
                    }
                  >
                    <div className="flex items-center space-x-3 mb-3">
                      <div
                        className={`p-2 rounded-lg ${testConfig.testPerformance ? 'bg-blue-500/20' : 'bg-gray-600/20'}`}
                      >
                        <Zap
                          className={`w-5 h-5 ${testConfig.testPerformance ? 'text-blue-400' : 'text-gray-400'}`}
                        />
                      </div>
                      <div className="flex-1">
                        <h4 className="font-semibold text-white">
                          性能测试 <span className="text-xs text-yellow-400">(可选)</span>
                        </h4>
                  ...
                  <div
                    ...
                    onClick={() =>
                      setTestConfig(prev => ({ ...prev, testReliability: !prev.testReliability }))
                    }
                  >
                    <div className="flex items-center space-x-3 mb-3">
                      <div
                        className={`p-2 rounded-lg ${testConfig.testReliability ? 'bg-green-500/20' : 'bg-gray-600/20'}`}
                      >
                        <Database
                          className={`w-5 h-5 ${testConfig.testReliability ? 'text-green-400' : 'text-gray-400'}`}
                        />
                      </div>
                      <div className="flex-1">
                  ...
                  <div
                    ...
                    onClick={() =>
                      setTestConfig(prev => ({
                        ...prev,
                        generateDocumentation: !prev.generateDocumentation,
                      }))
                    }
                  >
                    <div className="flex items-center space-x-3 mb-3">
                      <div
                        className={`p-2 rounded-lg ${testConfig.generateDocumentation ? 'bg-purple-500/20' : 'bg-gray-600/20'}`}
                      >
                        <FileText
                          className={`w-5 h-5 ${testConfig.generateDocumentation ? 'text-purple-400' : 'text-gray-400'}`}
                        />
                      </div>
                      <div className="flex-1">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <URLInput
                        ...
                        onChange={e =>
                          setTestConfig(prev => ({ ...prev, baseUrl: e.target.value }))
                        }
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="flex items-center space-x-3">
                        <input
                          ...
                          onChange={e =>
                            setTestConfig(prev => ({ ...prev, validateSchema: e.target.checked }))
                          }
                      ...
                      <div className="flex items-center space-x-3">
                        <input
                          ...
                          onChange={e =>
                            setTestConfig(prev => ({ ...prev, followRedirects: e.target.checked }))
                          }
                          className="w-4 h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-2"
                        />
                      ...
                      <div className="flex items-center space-x-3">
                        <input
                          ...
                          onChange={e =>
                            setTestConfig(prev => ({ ...prev, validateSSL: e.target.checked }))
                          }
                          className="w-4 h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-2"
                        />
                      ...
                      <div className="flex items-center space-x-3">
                        <input
                          ...
                          onChange={e =>
                            setTestConfig(prev => ({ ...prev, loadTest: e.target.checked }))
                          }
                          className="w-4 h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-2"
                        />
                      ...
                      <select
                        ...
                        onChange={e =>
                          setAuthentication(prev => ({
                            ...prev,
                            type: e.target.value as AuthenticationConfig['type'],
                          }))
                        }
                        title="选择API认证方式"
                    ...
                    {authentication.type === 'bearer' && (
                      <div>
                        ...
                        <input
                          ...
                          onChange={e =>
                            setAuthentication((prev: AuthenticationConfig) => ({
                              ...prev,
                        ...
                        >
                          <div className="flex items-center">
                            <input
                              ...
                              className="w-4 h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-2"
                            />
                            <label htmlFor={`header-enabled-${index}`} className="sr-only">
                              启用请求头
                            </label>
                          </div>
                          <div className="flex-1">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
                        <div className="md:col-span-2">
                          ...
                          <select
                            ...
                            onChange={e =>
                              updateEndpoint(endpoint.id, { method: e.target.value as any })
                            }
                            title="选择HTTP请求方法"
                        ...
                        <div className="md:col-span-2">
                          ...
                          <select
                            ...
                            onChange={e =>
                              updateEndpoint(endpoint.id, { priority: e.target.value as any })
                            }
                            title="设置端点测试优先级"
              ...
              {result && (
                <div className="bg-gray-800/80 backdrop-blur-sm rounded-xl border border-gray-700/50 p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="text-center p-4 bg-yellow-500/20 border border-yellow-500/30 rounded-lg">
                      <div className="text-2xl font-bold text-yellow-400">
                        {Math.round(result?.averageResponseTime || 0)}ms
                      </div>
                          ...
                          <div className="space-y-2">
                            <div className="flex justify-between items-center">
                              ...
                              <span className="text-green-400">
                                {result?.performanceMetrics.responseTimeDistribution?.fast || 0}
                              </span>
                            </div>
                            <div className="flex justify-between items-center">
                              <span className="text-xs text-gray-400">中等 (200-1000ms)</span>
                              <span className="text-yellow-400">
                                {result?.performanceMetrics.responseTimeDistribution?.medium || 0}
                              </span>
                            </div>
                            <div className="flex justify-between items-center">
                              <span className="text-xs text-gray-400">慢速 (&gt;1000ms)</span>
                              <span className="text-red-400">
                                {result?.performanceMetrics.responseTimeDistribution?.slow || 0}
                              </span>
                            </div>
                          </div>
                        </div>
                        ...
                        {result?.securityIssues.slice(0, 5).map((issue: any, index: number) => (
                          ...
                          >
                            ...
                            <div className="text-sm text-gray-300">
                              {typeof issue === 'string'
                                ? issue
                                : issue.description || issue.message || '安全问题'}
                            </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {(result?.endpointResults || result?.endpoints || []).map(
                            ...
                            <div className="flex items-center space-x-3">
                              <span
                                className={`px-2 py-1 text-xs rounded ${
                                  endpoint.performanceCategory === 'excellent'
                                    ? 'bg-green-600 text-white'
                                    : endpoint.performanceCategory === 'good'
                                      ? 'bg-blue-600 text-white'
                                      : endpoint.performanceCategory === 'fair'
                                        ? 'bg-yellow-600 text-white'
                                        : 'bg-red-600 text-white'
                                }`}
                              >
                              ...
                              <span
                                className={`px-2 py-1 text-xs font-semibold rounded ${getStatusColor(endpoint.status)}`}
                              >
                                {endpoint.statusCode}
                              </span>
                            </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                            <div>
                              ...
                              <div className="space-y-1">
                                <div className="text-gray-300">
                                  大小: {endpoint.responseSize || 0} bytes
                                </div>
                                <div className="text-gray-300">
                                  重试: {endpoint.retryCount || 0} 次
                                </div>
                                {endpoint.responseAnalysis && (
                                  <div className="text-gray-300">
                                    类型: {endpoint.responseAnalysis.contentType}
                                  </div>
                                )}
                              </div>
                            </div>

                            {endpoint.performanceIssues &&
                              endpoint.performanceIssues.length > 0 && (
                                <div>
                                    ...
                                    {endpoint.performanceIssues
                                      ...
                                      .map((issue: any, i: number) => (
                                        <div key={i} className="text-yellow-400 text-xs">
                                          {typeof issue === 'string'
                                            ? issue
                                            : issue.description || issue.message || '性能问题'}
                                        </div>
                                      ))}
                                  </div>
                                </div>
                            ...
                            {endpoint.securityIssues && endpoint.securityIssues.length > 0 && (
                              <div>
                                ...
                                <div className="space-y-1">
                                  {endpoint.securityIssues
                                    .slice(0, 2)
                                    .map((issue: any, i: number) => (
                                      <div key={i} className="text-red-400 text-xs">
                                        {typeof issue === 'string'
                                          ? issue
                                          : issue.type || issue.message || '安全问题'}
                                      </div>
                                    ))}
                                </div>
                              </div>
                          ...
                          {endpoint.error && (
                            <div className="mt-3 p-3 bg-red-500/10 border border-red-500/20 rounded">
                              ...
                              {endpoint.errorDiagnosis && (
                                <div className="text-xs text-gray-400">
                                  <div className="mb-1">
                                    建议: {endpoint.errorDiagnosis.suggestion}
                                  </div>
                                  {endpoint.errorDiagnosis.troubleshooting &&
                                    endpoint.errorDiagnosis.troubleshooting.length > 0 && (
                                      <div>
                                        排查:{' '}
                                        {endpoint.errorDiagnosis.troubleshooting
                                          .slice(0, 2)
