import { Activity, AlertCircle, ArrowLeft, BarChart3, Calendar, CheckCircle, Clock, Download, Settings, Share2, TrendingUp, Users, XCircle, Zap    } from 'lucide-react';import React, { useCallback, useEffect, useState    } from 'react';import { useNavigate, useParams    } from 'react-router-dom';import { Line, LineChart, ReferenceLine, ResponsiveContainer, Tooltip, XAxis, YAxis    } from 'recharts';// // // import '../../../components/stress/StatusLabel.css'; // 文件已删除 // 文件已删除 // CSS文件不存在，已注释'
import { DataProcessingUtils    } from '../../../utils/dataProcessingUtils.ts';// 智能数据采样函数 - 移到组件外部避免 hooks 顺序问题'
...
const StressTestDetail: React.FC  = () => {
  ...
  const exportData = useCallback(() => {
    ...
    const exportRecord = {
      ...
      grade: record.overallScore ? (record.overallScore >= 80 ? "A' : record.overallScore >= 60 ? 'B' : 'C') : "N/A','`
...
#压力测试 #性能测试 #TestWebApp`;`
  ...
  const sampleDataByInterval = (data: any[]) => {
    ...
    for (let windowStart = startTime; windowStart < endTime; windowStart += intervalMs) {
      ...
      if (windowData.length > 0) {
        // 计算窗口内的平均值
        const avgResponseTime = windowData.reduce((sum, item) => sum + (item.responseTime || item.avgResponseTime || item.response_time || 0), 0) / windowData.length;
        const avgThroughput = windowData.reduce((sum, item) => sum + (item.throughput || item.tps || item.requestsPerSecond || 0), 0) / windowData.length;
        const avgActiveUsers = windowData.reduce((sum, item) => sum + (item.activeUsers || 0), 0) / windowData.length;
        const avgErrorRate = windowData.reduce((sum, item) => sum + (item.errorRate || 0), 0) / windowData.length;
  ...
  // 数据验证和清理
  realTimeData = realTimeData.filter((item: any) => item &&
    typeof item === 'object' &&'
  ...
  return (<div className= 'min-h-screen bg-gray-900'>
      <div className= 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
        ...
        const startTime = new Date(finalChartData[0].timestamp).getTime();
                      ...
                      <div className= "flex items-center justify-center gap-6 mt-4 text-xs'>`
                        ...
                        {showAverage && (<div className= 'flex items-center gap-1'>
                            <div className= 'w-4 h-0.5 bg-yellow-500' style={{ borderTop: '2px dashed #f59e0b' }}></div>
                  ...
                  <div className= 'grid grid-cols-1 lg:grid-cols-2 gap-6'>
                    ...
                    <div className= 'bg-gray-800 rounded-lg p-6'>
                      ...
                      <div className= 'h-64'>
                        {finalChartData.length > 0 ? (<div className= 'flex items-end justify-between h-full gap-1'>
      ...
      {/* 重新测试确认对话框 */}
      {showRetestDialog && (<div className= 'fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
      ...
      {/* 分享结果对话框 */}
      {showShareDialog && (<div className= 'fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
          <div className= 'bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4'>
            <div className= 'flex items-center gap-3 mb-4'>
