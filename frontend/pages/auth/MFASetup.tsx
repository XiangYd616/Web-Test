...
const MFASetup: React.FC<MFASetupProps> = ({ onComplete, onCancel }) => {
  ...
  const renderStepContent = () => {
    switch (state.currentStep) {
      case 0:
        return (
          <Card title="开始设置双因素认证" className="mb-6">
            <Space direction="vertical" size="large" className="w-full">
              <div className="text-center">
                <SafetyOutlined className="text-6xl text-blue-500 mb-4" />
                <Title level={3}>保护您的账户安全</Title>
                <Paragraph className="text-gray-600">
                  双因素认证（MFA）为您的账户增加额外的安全层。即使密码泄露，没有您的手机认证应用，他人也无法登录您的账户。
                </Paragraph>
              </div>
              
              <div className="bg-blue-50 p-4 rounded-lg">
                <Title level={4}>您需要：</Title>
