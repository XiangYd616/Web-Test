...
// 测试状态类型定义
type TestStatusType = 'idle' | 'starting' | 'running' | 'completed' | 'failed' | 'cancelled';
...
const StressTest: React.FC = () => {
    ...
    // 状态转换验证函数
    const validateStatusTransition = (from: TestStatusType, to: TestStatusType): boolean => {
        const validTransitions: Record<TestStatusType, TestStatusType[]> = {
            'idle': ['starting', 'failed', 'completed'],
                    ...
                    if (testIdToCancel) {
                        ...
                        try {
                            const response = await fetch(`/api/test/stress/cancel/${testIdToCancel}`, {
                                ...
                                // fetch API不支持timeout，使用AbortController代替
                            });
    ...
    const [statusMessage,
        setStatusMessage] = useState<string>('准备开始测试');
    ...
    const calculateMetricsFromData = useCallback((data: TestDataPoint[]): RealTimeMetrics => {
        ...
        if (data.length > 0) {
            ...
            if (recentData.length > 1) {
                ...
                const timeSpanSeconds = Math.max(timeSpanMs / 1000, 1); // 至少1
                // TPS = 最近请求数 / 时间跨度（秒）
        ...
        if (data.length > 1) {
            ...
            const totalTimeSpanSeconds = Math.max(totalTimeSpanMs / 1000, 1); // 至少1
            // 平均TPS = 总请求数 / 总时间跨度（秒）
            averageTPS = Math.round((data.length / totalTimeSpanSeconds) * 10) / 10; // 保留1位小
        ...
        if (isRealTime) {
            ...
            setStressTestData(prev => {
                ...
                if (combined.length > maxDataPoints * 1.5) { // 超过预期50%才裁剪
                    const keepPoints = Math.floor(maxDataPoints * 0.9); // 保留90%的数据点
                    console.log(`📊 数据点过多（${combined.length} > ${maxDataPoints * 1.5}），裁剪到${keepPoints} 个数据点`);
    ...
    // 🔧 修复：转换TestDataPoint到EnhancedRealTimeData格式
    const convertToEnhancedRealTimeData = useCallback((dataPoints: TestDataPoint[]) => {
        return dataPoints.map(point => ({
            ...
            phase: point.phase === TestPhase.RAMP_UP ? 'rampup' :
                point.phase === TestPhase.RAMP_DOWN ? 'rampdown' : 'steady'
        }));
    }, []);
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (statusData.success && statusData.data) {
                    ...
                    // 设置基本指标 - 优先使用metrics，然后是realTimeMetrics
                    ...
                    if (statusData.data.realTimeData && statusData.data.realTimeData.length > 0) {
                        ...
                        console.log('📈 设置实时数据:', statusData.data.realTimeData.length, '个数据点');
    ...
    useEffect(() => {
        ...
        const checkRunningTests = async () => {
            try {
                ...
                console.error('🔧 检查运行中测试失败:', outerError);
            }
        };
    ...
    const startRealStressTest = async () => {
        ...
        try {
            ...
            if (isSuccess && testIdFromResponse) {
                ...
                // WebSocket房间加入将由connect事件自动处理，无需在此处重复发                
                if (testIdFromResponse) {
                    console.log('✅ 测试ID已确认', testIdFromResponse);
        ...
        useEffect(() => {
            ...
            const statusMapping: Record<string, TestStatusType> = {
                'IDLE': 'idle', 'PREPARING': 'starting',     // 准备阶段映射到启动中'WAITING': 'starting',       // 等待阶段映射到启动中'STARTING': 'starting','RUNNING': 'running','COMPLETING': 'running',     // 完成阶段仍算运行            'COMPLETED': 'completed','FAILING': 'running',        // 失败过程中仍算运行中'FAILED': 'failed','CANCELLING': 'running',     // 取消过程中仍算运行中'CANCELLED': 'cancelled','TIMEOUT': 'failed'          // 超时归类为失败
            };
        ...
        // 监听生命周期管理器的状态变- 已修复并启用
        useEffect(() => {

            if (lifecycleManager) {

                console.log('🔄 生命周期管理器已启用并准备就');
            }
        }, [lifecycleManager]);
        ...
        const unifiedTestData = {
            ...
            currentMetrics: metrics ? {
                ...
                // 添加数据传输相关的默认? dataReceived: metrics.dataReceived || 0,
        ...
        useEffect(() => {
            ...
            if (isRunning && currentTestId) {
                ...
                if (!isRunning && result) {
                    ...
                    if (!isRunning && !currentTestId && !result && !['completed', 'cancelled', 'failed'].includes(testStatus)) {
                        // 完全空闲状态?- 但不重置已完成的状态?
                        targetStatus = 'idle';
                    }
                    ...
                    switch (event) {
                        ...
                        case 'testCompleted':
                            ...
                            if (testInfo.realTimeData && testInfo.realTimeData.length > 0) {
                                ...
                                console.log('🏁 聚合数据已生成用于测试结果视', aggregatedChartData.length, '个数据点');
                            } else {
                                console.log('⚠️ No real-time data available for chart');
                            }

                            setResult(processedResult);
                            // 🔧 修复：检查当前状态，如果已经是取消状态则不覆                        
                            if (testStatus === 'cancelled') {
                                ...
                                setIsRunning(false);
                                // 🔧 修复：如果是取消状态，立即清理测试ID；否则延迟清                       
                                if ((testStatus as TestStatusType) === 'cancelled' || processedResult.status === 'cancelled') {
                                ...
                                if (processedResult) {
                                    ...
                                    if (currentRecord && processedResult.status !== 'cancelled' && (testStatus as TestStatusType) !== 'cancelled') {
                                        (async () => {
                                            try {
                        ...
                        case 'testCancelled':
                            setBackgroundTestInfo(testInfo);
                            setTestProgress('测试已取消');
                            setTestStatus('cancelled'); // 修复：使用正确的cancelled状态
                            setIsRunning(false);
                            setIsStopping(false);
                            // 🔧 修复：立即清理测试ID
                            setCurrentTestId(null);
            ...
            const initializeSocket = async () => {
                try {
                    ...
                    const socket = io('http://localhost:3001', {
                        ...
                        // 完全禁用自动重连，手动控? reconnection: false, // 禁用自动重连
                        autoConnect: false, // 禁用自动连接
                        forceNew: true, // 强制创建新连
                    ...
                    socket.on('test-progress', (data: any) => {
                        ...
                        if (data.testId === currentTestIdRef.current) {
                            ...
                            console.log('⚠️ test-progress 事件?testId 不匹', {
                            ...
                            fetch('/api/test/status')
                                ...
                                .then(statusData => {
                                    if (statusData.success && statusData.data && statusData.data.testId) {
                                        ...
                                        // 恢复测试状态                                    // 🔧 修复：只有在非终态时才恢复为running状态                                    
                                        if (!['completed', 'cancelled', 'failed'].includes(testStatus)) {
                                            setIsRunning(true);
                                            setTestStatus('running');
                                            setCurrentStatus('RUNNING');
                    ...
                    // 恢复重要?stress-test-data 监听?- 处理完整测试数据和状态?
                    socket.on('stress-test-data', (data) => {
                        ...
                        console.log('✅stress-test-data 事件已接收，开始处理数..');

                        // 检查testId是否匹配 - 使用ref获取最新                    
                        const currentTestIdValue = currentTestIdRef.current;
                        ...
                        if (data.testId !== currentTestIdValue) {
                            if (!currentTestIdValue && data.testId) {
                                ...
                                if (currentTestIdValue && data.testId !== currentTestIdValue) {
                                    // 只有在有testId但不匹配时才警告并返                            
                        ...
                        if (data.dataPoint) {
                            ...
                            console.log('📈 处理嵌套数据', dataPoint);
                        } else
                            if (data.dataPointTimestamp) {
                                // 格式2：直接的数据字段
                                dataPoint = {
                                    ...
                                    errorRate: 0, // 需要计 success: true
                                };

                                console.log('✅ 处理直接数据', dataPoint);
                            }
                        ...
                        if (metricsData && metricsData.totalRequests > 0) {
                            ...
                            console.log('🔄 使用后端提供的指标数?(优先):', updatedMetrics);
                            setMetrics(updatedMetrics);
                        } else {
                            // 只有在后端没有提供指标数据时才使用前端计
                    ...
                    socket.on('progress', (data) => {
                        ...
                        if (data.metrics) {
                            ...
                            const updatedMetrics: RealTimeMetrics = {
                                ...
                                throughput: data.metrics.throughput || 0, // 🔧 修复：使用后端提供的平均吞吐?
                                errorRate: data.metrics.errorRate || 0,
                                activeUsers: data.metrics.activeUsers || 0,
                                timestamp: Date.now(),
                                // 可选属 requestsPerSecond: data.metrics.requestsPerSecond || data.metrics.currentTPS || 0,
                            ...
                            console.log('✅ 更新指标', updatedMetrics);
                            setMetrics(updatedMetrics);
                        }
                    });
                ...
                const roomCheckInterval = setInterval(() => {
                    if (socketRef.current.connected && currentTestId) {
                        ...
                        if (stressTestData.length === 0) {
                            ...
                            console.log('✅ 重新加入房间', currentTestId);
        ...
        useEffect(() => {
            ...
            const checkForceCompletion = () => {
                ...
                console.log('🔍 强制完成检查', {
                    ...
                    timeSinceLastData: Math.floor(timeSinceLastData / 1000) + '',
                    shouldComplete: now > expectedEndTime && timeSinceLastData > 10000
                });

                // 如果当前时间超过预期结束时间，且最后数据超0秒，强制完成

                if (now > expectedEndTime && timeSinceLastData > 10000) {
                    console.log('🔧 强制完成测试：时间已到且数据流停');
                    setTestStatus('completed');
                    setTestProgress('压力测试完成');
                    setIsRunning(false);
                    setTimeout(() => setCurrentTestId(null), 2000);
        ...
        useEffect(() => {
            ...
            const checkTestCompletion = () => {
                ...
                if (lastDataPoint) {
                    ...
                    const detectionTimeout = Math.min(testDuration * 0.2, 15000); // 测试时长度?0%，最?5?
                    console.log('🔍 数据流检查', {
                        timeSinceLastData: timeSinceLastData / 1000 + '',
                        detectionTimeout: detectionTimeout / 1000 + '',
                        testDuration: testDuration + '秒'
                    });

                    if (timeSinceLastData > detectionTimeout) {
                        console.log(`🔍 检测到数据流停止超?{detectionTimeout / 1000}秒，检查测试状态`);
            ...
            const completionCheckInterval = setInterval(checkTestCompletion, 2000); // ?秒检查一次?
            // 🔧 新增：基于测试配置的主动完成检查
            ...
            const expectedEndTime = testStartTime + (expectedTestDuration * 1000) + 15000; // 额外15秒缓
            ...
            const expectedCompletionIntervalId = setInterval(checkExpectedCompletion, 3000); // ?秒检查预期完成时间?
            return () => {
        ...
        const resetTestState = useCallback(() => {
            ...
            setIsStopping(false);
            setError('');
            setBackgroundTestInfo(null); // 🔧 修复：重置后台测试信?
            // 重置数据
            setStressTestData([]);  // 🔧 清理唯一数据?
            setMetrics(null);
            setResult(null);
        ...
        const handleCancelConfirm = async (reason: string, preserveData: boolean) => {
                        ...
                        if (response.ok) {
                            ...
                            if (currentRecord) {
                                try {
                                    ...
                                    console.log('✅测试记录已标记为取消（后端失败但本地成功率');
                                } catch (recordError) {
                                    console.warn('⚠️ 更新取消记录失败:', recordError);
                                }
                            }
            ...
            const handleCancelProgressComplete = () => {
                ...
                // 🔧 修复：测试ID已在handleCancelConfirm中清理，这里不需要重复清        
            ...
            const calculateTestProgress = useCallback(() => {
                ...
                switch (testStatus) {
                    ...
                    case 'running':
                        ...
                        if (result.startTime) {
                            const startTime = new Date(result.startTime).getTime();
                            const elapsed = Math.max(0, (now - startTime) / 1000); // 已运行秒
                            // 🔧 改进：更精确的阶段划分和进度计算
                            ...
                            if (elapsed <= rampUpDuration) {
                                ...
                                if (elapsed <= rampUpDuration + mainTestDuration + cooldownDuration) {
                                    // 冷却阶段0% - 100%
                                    currentPhase = '冷却阶段';
                            ...
                            if (stressTestData.length > 0) {
                                ...
                                if (testStartTime && latestData.timestamp) {
                                    ...
                                    timeInfo = `已运?${formatTime(elapsed)} (${dataPoints} 数据?`;
                                } else {
                                    progress = 5 + (dataProgress * 95);
                                    timeInfo = `已收?${dataPoints} 个数据点`;
                                }
                            } else {
                                progress = 5 + (dataProgress * 95);
                                timeInfo = `已收?${dataPoints} 个数据点`;
                    ...
                    case 'completed':
                        ...
                        if (result.startTime && result.endTime) {
                            ...
                            timeInfo = `测试完成，总用?${formatCompletedTime(duration)}`;
                    ...
                    case 'cancelled':
                        ...
                        if (result.startTime) {
                            ...
                            const totalDuration = testConfig.duration + (testConfig.rampUp || 0) +
                                (testConfig.warmupDuration || 0) + (testConfig.cooldownDuration || 0);
                            const timeProgress = Math.min(elapsed / totalDuration, 1);
                            progress = 5 + (timeProgress * 95); // 基于实际时间的真实进行
                            ...
                            timeInfo = `测试已取消，运行?${formatCancelledTime(elapsed)}`;
                        } else {
            ...
            const handleApplyTemplate = (templateId: string) => {
                ...
                const templates: Record<string, Partial<StressTestConfig>> = {
                    'light-load': { users: 5, duration: 30, testType: 'gradual' }, 'medium-load': { users: 20, duration: 60, testType: 'gradual' }, 'heavy-load': { users: 50, duration: 120, testType: 'stress' }
                };
            ...
            const getRecommendedConfig = (testType: string) => {
                const configs = {
                    gradual: {
                        users: {
                            min: 1, max: Number.MAX_SAFE_INTEGER, recommended: 20, description: '逐步增加负载 - 无限制'
                        },
                        duration: {
                            min: 1, max: Number.MAX_SAFE_INTEGER, recommended: 180, description: '需要足够时间观察爬坡过程 - 无限制'
                        },
                        rampUp: {
                            min: 1, max: Number.MAX_SAFE_INTEGER, recommended: 30, description: '缓慢加压时间 - 无限制'
                        }
                    },
                    spike: {
                        users: {
                            min: 1, max: Number.MAX_SAFE_INTEGER, recommended: 50, description: '瞬间高并发冲击 - 无限制'
                        },
                        duration: {
                            min: 1, max: Number.MAX_SAFE_INTEGER, recommended: 60, description: '短时间高强度测试 - 无限制'
                        },
                        rampUp: {
                            min: 1, max: Number.MAX_SAFE_INTEGER, recommended: 5, description: '快速启动时间 - 无限制'
                        }
                    },
                    constant: {
                        users: {
                            min: 1, max: Number.MAX_SAFE_INTEGER, recommended: 30, description: '稳定持续负载 - 无限制'
                        },
                        duration: {
                            min: 1, max: Number.MAX_SAFE_INTEGER, recommended: 300, description: '长时间稳定性测试 - 无限制'
                        },
                        rampUp: {
                            min: 1, max: Number.MAX_SAFE_INTEGER, recommended: 10, description: '快速达到目标负载 - 无限制'
                        }
                    },
                    stress: {
                        users: {
                            min: 1, max: Number.MAX_SAFE_INTEGER, recommended: 100, description: '高强度压力测试 - 无限制'
                        },
                        duration: {
                            min: 1, max: Number.MAX_SAFE_INTEGER, recommended: 240, description: '充分的压力测试时间 - 无限制'
                        },
                        rampUp: {
                            min: 1, max: Number.MAX_SAFE_INTEGER, recommended: 30, description: '分阶段加压 - 无限制'
                        }
                    },
                    load: {
                        users: {
                            min: 1, max: Number.MAX_SAFE_INTEGER, recommended: 40, description: '模拟真实用户负载 - 无限制'
                        },
                        duration: {
                            min: 1, max: Number.MAX_SAFE_INTEGER, recommended: 600, description: '长时间真实场景测试 - 无限制'
                        },
                        rampUp: {
                            min: 1, max: Number.MAX_SAFE_INTEGER, recommended: 60, description: '模拟用户逐步进入 - 无限制'
                        }
                    },
                    volume: {
                        users: {
                            min: 1, max: Number.MAX_SAFE_INTEGER, recommended: 200, description: '大量数据处理测试 - 无限制'
                        },
                        duration: {
                            min: 1, max: Number.MAX_SAFE_INTEGER, recommended: 120, description: '高频请求测试 - 无限制'
                        },
                        rampUp: {
                            min: 1, max: Number.MAX_SAFE_INTEGER, recommended: 10, description: '快速达到最大容量 - 无限制'
                        }
                    }
                };
                                                            ...
                                                            <div className={`w-2 h-2 rounded-full ${testStatus === 'running' ? 'bg-green-500 animate-pulse' :
                                                                testStatus === 'completed' ? 'bg-blue-500' :
                                                                    testStatus === 'failed' ? 'bg-red-500' :
                                                                        testStatus === 'cancelled' ? 'bg-yellow-500' : 'bg-gray-500'
                                                                }`}></div>
...
"
                                    ...
                                    {activeTab === 'test' && (
                                        <div className="flex items-center bg-gray-700/50 rounded-md p-0.5">
                                            <button
                                                ...
                                                className={`px-3 py-1.5 text-sm font-medium rounded transition-all ${!isAdvancedMode
                                                                        ...
                                                                        <div className={`w-2 h-2 rounded-full ${testStatus === 'running' ? 'bg-blue-400 animate-pulse' :
                                                                            testStatus === 'completed' ? 'bg-green-400' :
                                                                                testStatus === 'cancelled' ? 'bg-yellow-400' :
                                                                                    testStatus === 'failed' ? 'bg-red-400' : 'bg-gray-400'
                                                                            }`}></div>
                                                ...
                                                {currentQueueId ? (
                                                    <div>
                                                        ...
                                                        <div className="text-sm text-blue-200">
                                                            队列位置: ?{getQueuePosition(currentQueueId)} ?                                            {estimateWaitTime(currentQueueId) > 0 && (
                                                                <span className="ml-2">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="text-sm text-blue-200">
                                                            当前?{queueStats.queueLength} 个测试在排队等待
                                                        </div>
                                                    </div>
                                                )}
                                            </div>
                                                            ...
                                                            <div className="text-center mt-6">
                                                                ...
                                                                <div className="text-xs text-blue-300 bg-blue-500/10 rounded-full px-2 py-1">
                                                                    {(() => {
                                                                        ...
                                                                        return fullTemplate ? `${fullTemplate.config.users}用户 · ${fullTemplate.config.duration}秒` : '配置加载..';
                                                                    })()}
                                                                </div>
                                                ...
                                                {selectedTemplate && (
                                                    <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4 mb-4">
                                                        ...
                                                        <div className="text-sm text-gray-300">
                                                            将使?<span className="text-blue-300 font-medium">{testConfig.users}</span> 个并发用户，
                                                    ...
                                                    {localStressTest.isAvailable && (
                                                        <div className="mb-4 p-4 bg-gradient-to-r from-purple-500/10 to-blue-500/10 border border-purple-500/20 rounded-lg">
                                                            ...
                                                            <div className="text-sm text-gray-300 mb-3">
                                                                使用您的本地计算机资源进行压力测试，突破服务器限制，支持更高并发数和更长测试时间?                                            </div>
                                                            {useLocalTest && (
                                                                    ...
                                                                    <div className="bg-blue-500/10 border border-blue-500/20 rounded p-2">
                                                                        ...
                                                                        <div className="text-gray-300 mt-1">
                                                                            {(() => {
                                                                                ...
                                                                                return `最大用? ${rec.users || testConfig.users}`;
                                                                            })()}
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                    ...
                                                    <div className="grid grid-cols-1 sm: grid-cols-2 lg:grid-cols-3 gap-4">
                                                        ...
                                                        <div>
                                                            <label className="block text-sm font-medium text-gray-300 mb-2">
                                                                并发用户数                                                <span className="text-xs text-gray-500 ml-2">
                                                        ...
                                                        <div className="space-y-3">
                                                            ...
                                                            <div className="flex items-center justify-between">
                                                                <div className="flex items-center space-x-2">
                                                                    ...
                                                                    <div className="text-xs text-gray-500">(可?</div>
                                                                </div>
                                                                <label className="relative inline-flex items-center cursor-pointer">
                                                                    <input
                                                                        type="checkbox"
                                                                    ...
                                                                    <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-3">
                                                                        <div className="flex items-center justify-between mb-2">
                                                                            ...
                                                                            <button
                                                                                ...
                                                                                title="测试代理连接"
                                                                            >
                                                                                {proxyTestStatus.testing ? '测试..' : '测试连接'}
                                                                            </button>
                                                                        </div>
                                                                        ...
                                                                        testConfig.testType === 'constant' ? 'bg-green-500/20 text-green-300' :
                                                                            testConfig.testType === 'stress' ? 'bg-purple-500/20 text-purple-300' :
                                                                                testConfig.testType === 'load' ? 'bg-orange-500/20 text-orange-300' :
                                                                                    testConfig.testType === 'volume' ? 'bg-yellow-500/20 text-yellow-300' : 'bg-gray-500/20 text-gray-300'
                                                                    }`}>
                                                                    {testConfig.testType === 'gradual' ? '梯度加压' :
                                                        ...
                                                        <div className="space-y-2">
                                                            ...
                                                            >
                                                                <div className="flex items-center justify-between">
                                                                    ...
                                                                    <span className="text-xs bg-gray-600/50 px-2 py-1 rounded">5用户/30</span>
                                                                </div>
                                                            ...
                                                            >
                                                                <div className="flex items-center justify-between">
                                                                    ...
                                                                    <span className="text-xs bg-gray-600/50 px-2 py-1 rounded">20用户/60</span>
                                                                </div>
                                                            ...
                                                            >
                                                                <div className="flex items-center justify-between">
                                                                    ...
                                                                    <span className="text-xs bg-gray-600/50 px-2 py-1 rounded">50用户/120</span>
                                                                </div>
                                                            ...
                                                            <div className="space-y-3 text-sm">
                                                                ...
                                                                <div className="flex items-center justify-between">
                                                                    <div className="flex items-center space-x-2">
                                                                        ...
                                                                        <span className="text-gray-300">实时错误检查</span>
                                                                    </div>
                                                    ...
                                                    {(stressTestData && stressTestData.length > 0) ? (
                                                        <div>
                                                            <div className="mb-2 text-sm text-gray-400">
                                                                传统图表模式 (数据? {stressTestData.length})
                                                                {isRunning && <span className="ml-2 text-green-400">?运行</span>}
                                                            </div>
                                                            ...
                                                            <div className="flex items-center justify-center h-full">
                                                                <div className="text-center">
                                                                    ...
                                                                    <div className="text-gray-500 text-xs mb-4">
                                                                        数据: {stressTestData.length} | WebSocket: {socketRef.current.connected ? '已连接' : '未连接'}
                                                                    </div>
                                        ...
                                        {isRunning && (
                                            <div className="bg-gray-800/80 backdrop-blur-sm rounded-xl border border-gray-700/50 p-6">
                                                ...
                                                <div className="bg-gray-900/80 text-green-400 p-4 rounded-lg font-mono text-sm h-32 overflow-y-auto border border-gray-700">
                                                    ...
                                                    <div>[{new Date().toLocaleTimeString()}] 📊 配置: {testConfig.users}用户, {testConfig.duration}</div>
                            ...
                            <CancelTestConfirmDialog
                                ...
                                testProgress={isRunning ? {
                                    ...
                                    phase: currentStatus === 'RUNNING' ? '压力测试运行' :
                                        currentStatus === 'STARTING' ? '测试启动' :
