...
const AnalyticsOverview = () => (
  <div className="p-4 bg-gray-100 rounded">分析概览组件开发中...</div>
);
const ImportExport = () => <div className="p-4 bg-gray-100 rounded">导入导出组件开发中...</div>;
const StreamingMonitoring = () => (
  <div className="p-4 bg-gray-100 rounded">实时监控组件开发中...</div>
);
...
const DataStorage: React.FC = () => {
  ...
  const applyFilters = () => {
    ...
    if (filters.dateRange !== 'all') {
      ...
      filtered = filtered.filter(record => new Date(record.startTime) >= filterDate);
    }
    ...
    filtered = filtered.filter(record => {
      ...
      return (
        record.overallScore >= filters.scoreRange[0] && record.overallScore <= filters.scoreRange[1]
      );
    });
                ...
                <div className="text-2xl font-bold text-blue-400">
                  {Math.round(
                    testRecords
                      ...
                      testRecords.filter(r => r?.overallScore).length
                  ) || 0}
                </div>
                        ...
                        <div className="flex items-start space-x-4">
                          ...
                          <div className="flex-1">
                            <header className="flex items-center space-x-3 mb-2">
                              ...
                              {record.overallScore && (
                                <span
                                  className={`text-sm font-bold ${getScoreColor(record.overallScore)}`}
                                >
                            ...
                            <div className="flex items-center space-x-4 text-xs text-gray-500">
                              ...
                              <span>
                                开始: {new Date(record.startTime).toLocaleString('zh-CN')}
                              </span>
