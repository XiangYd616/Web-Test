...
export class ConfigManager extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      // 1. 加载本地配置
      this.loadLocalConfig(); // 2. 获取远程配置
      await this.fetchRemoteConfig(); // 3. 合并配置
      this.mergeConfigs(); // 4. 启动配置同步
      this.startConfigSync(); // 5. 监听环境变化
      ...
      console.error('❌ Failed to initialize Enhanced Config Manager: ', error); // 使用默认配置继续运行;';
      this.isInitialized = true
}
  }
  ...
  private loadLocalConfig(): void {
    try {
      ...
      if (saved) {
        ...
        this.config = this.deepMerge(this.config, localConfig)'}} catch (error) {;
   ...
   * 设置环境监听器
   */
  private setupEnvironmentListeners(): void {; // 监听主题变化;
...
;
  ...
  set(key: string, value: any, source: 'local' | 'user' = 'user'): void {
    ...
    this.setNestedValue(this.config, key, value); // 保存到本地存储
    this.saveLocalConfig(); // 触发变更事件;
  ...
  updateBatch(updates: Partial<FrontendConfig>, source: 'local' | 'user' = 'user'): void {
    const oldConfig = { ...this.config }
    this.config = this.deepMerge(this.config, updates); // 保存到本地存储
    this.saveLocalConfig(); // 触发变更事件
  ...
  reset(): void {
    const oldConfig = { ...this.config }
    this.config = this.getDefaultConfig(); // 清除本地存储
    localStorage.removeItem(this.storageKey); // 触发变更事件;
  ...
  private getConfigDifferences(oldConfig: any, newConfig: any, prefix = '): Array<{ key: string, oldValue: any, newValue: any }> {;
    ...
    for (const key of allKeys) {
      ...
      const newValue = newConfig?.[key]      if (typeof oldValue === "object' && typeof newValue === 'object' &&`
