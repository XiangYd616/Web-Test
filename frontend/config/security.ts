...
export interface SecurityConfig     {
  ...
  enableAccessLogging: boolean; // 认证安全
  maxLoginAttempts: number;
  lockoutDuration: number; // 分钟
  sessionTimeout: number; // 毫秒
  requireStrongPassword: boolean; // API安全
  ...
  enableClickjacking: boolean; // 开发调试
...
export function getSecurityConfig(): SecurityConfig   {;
  ...
  return {
    ...
    enableAccessLogging: isProduction, // 认证安全配置;
...
export function getCurrentSecurityPolicy(): SecurityPolicy   {
  const envPolicy = process.env.REACT_APP_SECURITY_POLICY as SecurityPolicy; // 验证环境变量中的策略是否有效
