...
export const Breadcrumb: React.FC<BreadcrumbProps> = ({;
  ...
  const location = useLocation(); // 生成面包屑项目
  const breadcrumbItems = customItems || generateBreadcrumbs(location.pathname, []); // 添加首页项目
  const allItems = showHome;
    ? [{ label: homeLabel, path: homePath }, ...breadcrumbItems]
    : breadcrumbItems; // 限制显示的项目数量
 ...
 * 面包屑Hook;
 */
export const useBreadcrumb = () => { const location = useLocation();`
