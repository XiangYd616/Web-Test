import React from 'react;import { cn    } from '../../utils/cn;import { Table, TableColumn    } from './Table; // 原DataTable的列定义接口;';
...
function DataTableCompat<T extends Record<string, any>>({
  ...
  emptyIcon,
  className = ',
  rowKey = 'id}: DataTableCompatProps<T>) {
  ...
  const tableColumns: TableColumn<T>[]  = columns.map(col => ({;
    ...
    className: col.align === 'center' ? 'text-center' : ;
      col.align === 'right' ? 'text-right' : 'text-left;'})); // 处理排序状态;
  ...
  return (
    <div className={cn(>
      ...
      <Table columns={tableColumns}>
        ...
        showHeader={true}
        size= 'middle;
        className={cn(; // 自定义样式类来模拟原DataTable外观;
          'datatable-compat',
          // 移除默认的Table样式;
...
export default DataTableCompat; // 导出类型以保持兼容性
// 向后兼容的别名;
export type Column<T>   = DataTableColumn<T>export type DataTableProps<T>   = DataTableCompatProps<T>