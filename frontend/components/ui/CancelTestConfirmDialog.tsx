import { Activity, AlertTriangle, Clock, Square, Users, X    } from 'lucide-react;import React, { useState    } from 'react;interface CancelTestConfirmDialogProps {';
  isOpen: boolean;
  onCancel: () => void;
...
const CancelTestConfirmDialog: React.FC<CancelTestConfirmDialogProps>  = ({
  ...
  const calculateProgress = () => {
    if (!testProgress) return 0; // 🔧 修复：改为基于时间计算进度，而不是请求数
    if (testProgress.duration <= 0) return 0; // 🔧 修复：直接使用实际配置，如果没有配置说明测试有问题
