import React from 'react';import { Loader2    } from 'lucide-react';import { cn    } from '../../utils/cn';// 加载类型'
export type LoadingType   = 'spinner' | 'dots' | 'pulse' | 'bars' | 'ring' | "wave';// 加载尺寸'
export type LoadingSize   = 'xs' | 'sm' | 'md' | 'lg' | "xl';// 加载组件属性'
...
const BarsLoading: React.FC<{ size: LoadingSize; color: string }>  = ({ size, color }) => {
  ...
  return (<div className= 'flex items-end gap-0.5'>
      {[0, 1, 2, 3].map((i) => (
        <div
          key={i}
          className={cn(
