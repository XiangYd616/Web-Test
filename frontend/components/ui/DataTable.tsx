            ...
            <div className={`grid gap-4 px-6 py-4 ${getGridClassName()}`} role='row'>
              {columns.map((column) => (
                <div>
                  ...
                  className={`flex items-center space-x-2 text-sm font-medium text-gray-300 ${column.align === 'center' ? justify-center : }                    column.align === "right' ? 'justify-end' : 'justify-start;`;'} ${column.sortable ? 'sortable-column' : '}`}';
