import { Activity, BarChart3, Download, ExternalLink, Eye, RefreshCw, Search, Trash2    } from 'lucide-react';import React, { useEffect, useRef, useState    } from 'react';import { useNavigate    } from 'react-router-dom';import { useAuth    } from '../../contexts/AuthContext';import ExportUtils from '../../utils/exportUtils';import { calculateTestCompletion, getStatusConfig, getStatusStyleClasses, // getStatusText   } from '../../utils/testStatusUtils';// 已修复'
import { DeleteConfirmDialog    } from '../ui/DeleteConfirmDialog.tsx';import ExportModal from '../ui/ExportModal.tsx';import { showToast    } from '../ui/Toast.tsx';import StressTestDetailModal from './StressTestDetailModal';import '../../styles/pagination.css';
...
const StressTestHistory: React.FC<StressTestHistoryProps>  = ({ className = "' }) => {'
  ...
  const getStatusIcon = (status: string) => {
    ...
    return (
      <IconComponent className={`w-4 h-4 ${isAnimated ? 'animate-pulse' : "'}`}'`
         />
    );
  };
  ...
  if (!isAuthenticated) {
        ...
        return (<div className={`test-records-container bg-gray-800/30 dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700/40 dark:border-gray-600/30 shadow-lg ${className`}
      }`}>`
  ...
  return (
    <div className={`test-records-container bg-gray-800/30 dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700/40 dark:border-gray-600/30 shadow-lg ${className}`}>`
      ...
      <div className= 'p-6'>
        {loading ? (
          ...
          <div className= 'text-center py-12'>
            ...
            {/* 分页组件 */}
            {totalRecords > 0 && (<div className= 'mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 p-4 bg-gray-800/20 dark:bg-gray-800/40 backdrop-blur-sm border border-gray-700/30 rounded-lg'>
                {/* 分页信息 */}
      ...
      {/* 删除确认对话框 */}
      <DeleteConfirmDialog isOpen={deleteDialog.isOpen}
