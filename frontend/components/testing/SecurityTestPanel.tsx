...
import { AlertTriangle, Award, Eye, FileText, Lock, Network, Settings, Shield, Target, Zap    } from 'lucide-react';import React, { forwardRef, useCallback, useImperativeHandle, useState    } from 'react';import { SecurityTestConfig, SecurityTestResult, TestProgress, unifiedSecurityEngine    } from '../../services/auth/securityEngine';// import { createCommonErrors, createError   } from '../../utils/errorHandler';// 函数不存在，已注释'
import { URLValidationResult    } from '../../utils/urlValidator';import { Error    } from './ErrorDisplay';interface SecurityTestPanelProps   {'
  onTestStart?: () => void;
...
export const SecurityTestPanel = forwardRef<SecurityTestPanelRef, SecurityTestPanelProps>(({
  ...
  const runTest = useCallback(async () => {
    ...
    try {
      const result = await unifiedSecurityEngine.runSecurityTest(config,
        (progressData) => {
