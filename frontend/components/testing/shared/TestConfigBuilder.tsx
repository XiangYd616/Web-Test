...
import { TestConfigField, TestConfigSchema, TestConfigSection } from '../ComprehensiveTestPage';
...
export const TestConfigBuilder: React.FC<TestConfigBuilderProps> = ({
  ...
  // 检查字段依赖
  const checkFieldDependencies = useCallback(
    (field: TestConfigField): boolean => {
  ...
  const renderFieldInput = (field: TestConfigField, value: unknown, fieldId: string) => {
    const baseClasses =
      'w-full px-3 py-2 bg-gray-700 dark:bg-gray-800 text-white border border-gray-600 dark:border-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 placeholder-gray-400';
  ...
  const renderSection = (section: TestConfigSection) => {
    ...
    const sectionFields = schema.fields.filter(field => section.fields.includes(field.key));
