import { AlertTriangle, CheckCircle, ChevronDown, ChevronRight, Download, Eye, FileText, TrendingUp, XCircle    } from 'lucide-react';import React, { useState    } from 'react';import { useTheme    } from '../../contexts/ThemeContext';interface LocalSEOResultsProps   {'
  results: any;
...
const LocalSEOResults: React.FC<LocalSEOResultsProps>  = ({ results, onExport }) => {
  ...
  const renderFiles = () => (
    <div className= 'space-y-4'>
      {results.fileAnalysis?.map((file: any, index: number) => (<div
          key={index}
        ...
        >
          ...
          {expandedFiles.has(file.filename) && (<div className= 'px-4 pb-4 border-t'>
              {file.analysis && (
                <div className= 'mt-4 space-y-4'>
                  ...
                  {/* 问题列表 */}
                  {file.issues && file.issues.length > 0 && (<div>
  ...
  const renderIssues = () => (<div className= 'space-y-4'>
      {results.issues?.length === 0 ? (
  ...
  const renderRecommendations = () => (<div className= 'space-y-4'>
      {results.recommendations?.length === 0 ? (
        <div className= 'text-center py-8 text-gray-500'>
