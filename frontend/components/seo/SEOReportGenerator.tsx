...
export interface SEOReportGeneratorHandle {
  exportReport: (format: 'pdf' | 'json' | 'html') => Promise<void>;
}

export const SEOReportGenerator = forwardRef<SEOReportGeneratorHandle, SEOReportGeneratorProps>(
  ({ reportData, onReportGenerated, onError }, ref) => {
    ...
    useImperativeHandle(
      ref,
      () => ({
        exportReport: async (format: 'pdf' | 'json' | 'html') => {
          switch (format) {
            case 'pdf':
      ...
      [generatePDFReport, generateJSONReport, generateHTMLReport]
    );
    ...
    const generateReportSummary = () => {
      ...
      if (reportData.basicSEO) {
        ...
        summary.criticalIssues += reportData.basicSEO.issues.filter(
          i => i.impact === 'high'
        ).length;
      }
        ...
        <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="text-center p-3 bg-red-50 rounded-lg">
            <div className="text-2xl font-bold text-red-600">
              {(reportData.basicSEO?.issues ?? []).filter(i => i.impact === 'high').length}
            </div>
...
SEOReportGenerator.displayName = 'SEOReportGenerator';
