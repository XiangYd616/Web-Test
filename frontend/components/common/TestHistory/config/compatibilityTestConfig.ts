...
export const compatibilityTestConfig: TestHistoryConfig = {
  testType: 'compatibility', apiEndpoint: '/api/test/compatibility', title: '兼容性测试历史', description: '查看和管理跨浏览器/设备兼容性测试记录',
  columns: [
    ...
    { key: 'browsers', title: '浏览器数', width: 100, sortable: true, align: 'center',
      formatter: (v: any) => v?.length || 0 },
    { key: 'passed', title: '通过数', width: 100, sortable: true, align: 'right',
      formatter: (n: number) => (n ?? 0).toLocaleString() },
    { key: 'failed', title: '失败数', width: 100, sortable: true, align: 'right',
      formatter: (n: number) => (n ?? 0).toLocaleString() },
    { key: 'compatibility', title: '兼容性', width: 120, sortable: true, align: 'center',
  ...
  customFilters: [
    { key: 'browser', label: '浏览器', type: 'select', options: [
      { value: 'all', label: '全部' }, { value: 'chrome', label: 'Chrome' }, { value: 'firefox', label: 'Firefox' },
      { value: 'safari', label: 'Safari' }, { value: 'edge', label: 'Edge' },
    ]},
    { key: 'minCompatibility', label: '最低兼容性', type: 'number', placeholder: '0-100(%)' },
  ],
  customActions: [
    { key: 'viewMatrix', label: '兼容性矩阵', onClick: (r) => { console.log('查看兼容性矩阵:', r); }, visible: r => r.status==='completed' },
  ],
  ...
  emptyState: { title: '暂无兼容性测试记录', description: '创建跨浏览器/设备兼容性测试', action: { label: '创建兼容性测试', onClick: () => { window.location.href = '/testing/compatibility/create'; } } },
};
