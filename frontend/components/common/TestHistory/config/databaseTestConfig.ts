...
export const databaseTestConfig: TestHistoryConfig = {
  ...
  columns: [
    { key: 'testName', title: '测试名称', width: 200, sortable: true, filterable: true },
    {
      ...
      sortable: true,
      formatter: (t: string) =>
        ({ mysql: 'MySQL', postgresql: 'PostgreSQL', mongodb: 'MongoDB', redis: 'Redis' })[t] ||
        t ||
        '-',
    },
    {
  ...
  emptyState: {
    title: '暂无数据库测试记录',
    description: '创建数据库性能和压力测试',
    action: {
      label: '创建数据库测试',
      onClick: () => {
        window.location.href = '/testing/database/create';
      },
    },
  },
};
