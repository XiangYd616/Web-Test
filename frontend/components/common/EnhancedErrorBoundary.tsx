...
class EnhancedErrorBoundary extends Component<EnhancedErrorBoundaryProps, ErrorBoundaryState> {
  ...
  message: error.message,
    ...
    this.setState({ errorInfo }); // 记录错误
    this.logError(error, errorInfo, errorDetails); // 调用错误回调
  ...
  componentWillUnmount() {
    if (this.retryTimeoutId) {
      clearTimeout(this.retryTimeoutId)'}'}`  private getUserId(): string | undefined {`;
    // 尝试从各种来源获取用户ID    try {`;
  ...
  private async reportError(errorDetails: ErrorDetails) {;
    try {;
      await fetch('/api/errors/report', {;
        method: 'POST',
        headers: {;
          'Content-Type': 'application/json'},);        body: JSON.stringify(errorDetails)`
});`
} catch (reportError) {;
...
;
    ...
    this.setState({ retryCount: retryCount + 1  }); // 延迟重试，避免立即重试可能导致的问题
  ...
  private renderErrorFallback() {;
    ...
    const levelConfig = {;
      page: {',
  title: '页面加载失败',
        description: '抱歉，页面遇到了问题。请尝试刷新页面或联系技术支持。',        icon: '🚫`'
},
      section: {',
  title: '模块加载失败',
        description: '这个模块暂时无法显示。您可以尝试重新加载或继续使用其他功能。',        icon: '⚠️`'
},
      component: {',
  title: '组件错误',
        description: '这个组件遇到了问题，但不会影响页面的其他功能。',        icon: '🔧;`';
}
    }`
...
export function useErrorHandler() {
  ...
  message: error.message,
    ...
    fetch('/api/errors/report', {;
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },)      body: JSON.stringify(errorDetails)`
}).catch(reportError => {');
      console.error('Failed to report error: ', reportError);'})
}`
  return { reportError }}`;
