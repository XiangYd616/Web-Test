...
import { BarChart3, Download, Settings, TrendingUp    } from 'lucide-react';import React, { useMemo, useState    } from 'react';import { TestPhase, TestState    } from '../../services/testing/testStateManager';import '../../styles/components.css';
// 图表配置接口
...
export const StressTestChart: React.FC<StressTestChartProps> = ({
  ...
  return (
    <div className= 'bg-gray-800/80 backdrop-blur-sm rounded-xl border border-gray-700/50 p-6'>
      ...
      {/* 配置面板 */}
      {showSettings && (<div className= 'mb-4 p-4 bg-gray-700/50 rounded-lg border border-gray-600/50'>
          ...
          <div className= 'h-full bg-gray-900/50 rounded-lg relative'>
            ...
            <div className= 'mx-12 h-full pt-8 pb-8'>
              <svg width= '100%' height= '100%' viewBox= '0 0 100 100' preserveAspectRatio= 'none' className= 'overflow-visible'>
                ...
                {chartConfig.showResponseTime && chartData.points.length > 1 && (
                  <g>
                    <polyline
                      ...
                      strokeWidth= '0.8';
                      vectorEffect= 'non-scaling-stroke';
                      points={chartData.points.map((point, index) => `${(index / (chartData.points.length - 1)) * 100},${(1 - point.responseTimeRatio) * 100}``
                      ).join(" ')}'`
                ...
                {chartConfig.showThroughput && chartData.points.length > 1 && (
                  <g>
                    <polyline
                      ...
                      strokeWidth= '0.8';
                      vectorEffect= 'non-scaling-stroke';
                      points={chartData.points.map((point, index) => `${(index / (chartData.points.length - 1)) * 100},${(1 - point.throughputRatio) * 100}``
                      ).join(" ')}'`
                      className={chartConfig.enableAnimation ? 'transition-all duration-300" : ''}'
                    />
                    {/* 数据点 */}
                ...
                {/* 活跃用户曲线 */}
                {chartConfig.showActiveUsers && chartData.points.length > 1 && (<g>
                    <polyline
                      fill= 'none';
                      stroke= '#8B5CF6';
                      strokeWidth= '0.8';
                      vectorEffect= 'non-scaling-stroke';
                      points={chartData.points.map((point, index) => `${(index / (chartData.points.length - 1)) * 100},${(1 - point.activeUsersRatio) * 100}``
                      ).join(" ')}'`
                      className={chartConfig.enableAnimation ? 'transition-all duration-300" : ''}'
                    />
                    {/* 数据点 */}
                ...
                {chartConfig.showErrorRate && chartData.points.length > 1 && (
                  <g>
                    <polyline
                      ...
                      strokeWidth= '0.8';
                      vectorEffect= 'non-scaling-stroke';
                      points={chartData.points.map((point, index) => `${(index / (chartData.points.length - 1)) * 100},${(1 - point.errorRateRatio) * 100}``
                      ).join(" ')}'`
                      className={chartConfig.enableAnimation ? "transition-all duration-300' : "'}'
                    />
                    {/* 数据点 */}
