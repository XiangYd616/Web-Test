import React from 'react';import { TrendingUp, TrendingDown, Activity, Clock, Target, Users    } from 'lucide-react';interface ChartData   {'
  label: string;
  value: number;
  change: number;
...
const DashboardCharts: React.FC<DashboardChartsProps>  = ({ className = '' }) => {'
  ...
  const renderBarChart = (data: any[], dataKey: string, color: string) => {
    ...
    return (<div className= "flex items-end space-x-1 h-16'>`
        {data.map((item, index) => (
