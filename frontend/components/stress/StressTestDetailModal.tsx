...
const StressTestDetailModal: React.FC<StressTestDetailModalProps> = React.memo(
  ({ record, isOpen, onClose }) => {
    ...
    const statusInfo = React.useMemo(
      () =>
        record
          ...
          : { icon: null, color: '', bg: '', text: '', description: '' },
      [record?.status]
    );
    ...
    const errorRate = React.useMemo(() => {
      if (!record) return '0%';
      const rate =
        record?.results?.metrics?.errorRate ||
        record?.errorRate ||
      ...
      record?.results?.metrics?.errorRate,
      record?.errorRate,
      record?.results?.metrics?.failedRequests,
      record?.results?.metrics?.totalRequests,
    ]);

    // 优化格式化的持续时间计算
    const formattedDuration = React.useMemo(
      () => (record ? formatDuration(record) : '-'),
      [record]
    );

    // 优化格式化的日期计算
    const formattedDate = React.useMemo(
      () => (record ? formatDate(record?.start_time || record?.created_at) : '-'),
                  ...
                  <div className="bg-gray-800 rounded-lg p-4">
                    ...
                    <p
                      className={`text-2xl font-bold ${
                        record?.performanceGrade.startsWith('A')
                          ? 'text-green-400'
                          : record?.performanceGrade.startsWith('B')
                            ? 'text-blue-400'
                            : record?.performanceGrade.startsWith('C')
                              ? 'text-yellow-400'
                              : 'text-red-400'
                      }`}
                    >
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {record?.tags.map((tag: string, index: number) => (
                      <span
                        key={index}
                        className="inline-flex items-center px-3 py-1 text-sm font-medium rounded-full bg-blue-600/60 text-blue-200 border border-blue-500/50"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-white font-medium">
                      {record?.config?.rampUpTime || record?.config?.rampUp || '-'}秒
                    </p>
                  </div>
                  <div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        <span
                          className={`w-2 h-2 rounded-full ${record?.config.proxy.enabled ? 'bg-green-500' : 'bg-gray-500'}`}
                        ></span>
                        <span
                          className={`text-sm font-medium ${record?.config.proxy.enabled ? 'text-green-400' : 'text-gray-400'}`}
                        >
                      ...
                      <>
                        <div className="flex items-center gap-4">
                          ...
                          <span className="text-white font-medium">
                            {record?.config.proxy.type?.toUpperCase() || 'HTTP'}
                          </span>
                        </div>
                        <div className="flex items-center gap-4">
                  ...
                  <div className="space-y-3">
                    {record?.config?.warmupDuration && (
                      <div className="flex items-center gap-4">
                        ...
                        <span className="text-white font-medium">
                          {record?.config.warmupDuration}秒
                        </span>
                      </div>
                    )}
                    {record?.config?.cooldownDuration && (
                      <div className="flex items-center gap-4">
                        ...
                        <span className="text-white font-medium">
                          {record?.config.cooldownDuration}秒
                        </span>
                      </div>
                    )}
    ...
    const modalContent = (
      <div className="stress-test-modal-container">
        {/* 背景遮罩 - 覆盖整个视口 */}
        <div className="stress-test-modal-backdrop" onClick={onClose} />

        {/* 模态窗口 - 始终在用户当前视口中心 */}
        <div className="stress-test-modal-content" onClick={e => e?.stopPropagation()}>
