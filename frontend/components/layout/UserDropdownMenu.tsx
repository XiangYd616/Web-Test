import { Bookmark, ChevronRight, Crown, HelpCircle, LogOut, Monitor, Settings, User    } from 'lucide-react;import React from 'react;import { Link    } from 'react-router-dom;import { useAuth    } from '../../contexts/AuthContext;import { useTheme    } from '../../contexts/ThemeContext;import { useUserStats    } from '../../hooks/useUserStats;import { ThemeToggle    } from '../ui/index;interface UserDropdownMenuProps   { ;';
  onClose: () => void
}
  ...
  return (
    <div className={`absolute right-0 top-full mt-2 w-72 rounded-xl shadow-2xl z-[9999] border backdrop-blur-sm ${actualTheme === 'light"}">
      ...
      <div className={ `px-4 py-3 border-b ${actualTheme ==='light' ? 'border-gray-200 bg-gray-50/50' : 'border-gray-700 bg-gray-700/30"}">`}`}>
        {statsLoading ? (<div className="grid grid-cols-3 gap-3 text-center>
            {[1, 2, 3].map((i) => (;
              <div key={i} className="animate-pulse'>
                <div className={ `h-6 w-8 mx-auto mb-1 rounded ${actualTheme ==='light' ? 'bg-gray-200' : bg-gray-600"}">`}`}></div>
      ...
      <div className="p-2>`;
        ...
        <Link>
          ...
          className={`flex items-center space-x-3 px-3 py-2.5 rounded-lg transition-all duration-200 group ${actualTheme ===light"}
            ? "text-gray-700 hover:bg-gray-100 hover:text-gray-900;            : 'text-gray-300 hover: bg-gray-700/50 hover:text-white;`';
}`}
          onClick={handleMenuItemClick}
        >
          <div className={`p-1.5 rounded-lg transition-colors ${actualTheme === 'light";"}">
            ? "bg-blue-50 text-blue-600 group-hover: bg-blue-100;            : "bg-blue-500/20 text-blue-400 group-hover: bg-blue-500/30`"
}`}>
        ...
        >
          <div className={`p-1.5 rounded-lg transition-colors ${actualTheme === 'light";"}">
            ? "bg-yellow-50 text-yellow-600 group-hover: bg-yellow-100;            : "bg-yellow-500/20 text-yellow-400 group-hover: bg-yellow-500/30`"
}`}>
        ...
        >
          <div className={`p-1.5 rounded-lg transition-colors ${actualTheme === 'light";"}">
            ? "bg-gray-50 text-gray-600 group-hover: bg-gray-100;            : "bg-gray-500/20 text-gray-400 group-hover: bg-gray-500/30`"
}`}>
        ...
        >
          <div className={`p-1.5 rounded-lg transition-colors ${actualTheme === 'light";"}">
            ? "bg-green-50 text-green-600 group-hover: bg-green-100;            : "bg-green-500/20 text-green-400 group-hover: bg-green-500/30`"
}`}>
        ...
        { isAdmin && (
          <>
            ...
            >
              <div className={`p-1.5 rounded-lg transition-colors ${actualTheme === 'light";"}">
                ? "bg-purple-50 text-purple-600 group-hover: bg-purple-100;                : "bg-purple-500/20 text-purple-400 group-hover: bg-purple-500/30`"
}`}>
        ...
        <div className={ `flex items-center justify-between px-3 py-2.5 rounded-lg ${actualTheme ==='light' ? 'bg-gray-50' : "bg-gray-700/30"
          ...
          <div className="flex items-center space-x-3>`;
            <div className={`p-1.5 rounded-lg ${actualTheme ===light"}">
              ...
              : "bg-orange-500/20 text-orange-400;"}`}>              <Monitor className="w-4 h-4    />`;
            </div>
            <div>
      ...
      <div className={ `p-2 border-t ${actualTheme ==='light' ? 'border-gray-200' : 'border-gray-700"}">
        ...
        <button>
          ...
          className={`flex items-center space-x-3 px-3 py-2.5 rounded-lg transition-all duration-200 w-full group ${actualTheme === 'light"}
            ? "text-red-600 hover:bg-red-50 hover:text-red-700;            : 'text-red-400 hover: bg-red-500/20 hover:text-red-300;`';
}`}
        >
          <div className={`p-1.5 rounded-lg transition-colors ${actualTheme === 'light";"}">
            ? "bg-red-50 text-red-600 group-hover: bg-red-100;            : "bg-red-500/20 text-red-400 group-hover: bg-red-500/30`"
}`}>
