...
import { AlertTriangle, CheckCircle, ExternalLink, Globe, HelpCircle, RefreshCw, Zap    } from 'lucide-react';import React, { useCallback, useEffect, useMemo, useState    } from 'react';import { URLValidationResult, validateUrlSync    } from '../../utils/urlValidator';interface UrlInputProps   {'
  value: string;
...
export const UrlInput: React.FC<UrlInputProps> = ({
  ...
  return (<div className={`space-y-3 ${className}`}>`
      ...
      {/* 验证结果显示 */}
      {validationResult && (<div className= 'space-y-2'>
          {/* 自动修复提示 */}
          ...
          {/* 错误信息 */}
          {validationResult.errors.length > 0 && (<div className= 'p-3 bg-red-900/20 border border-red-800/50 rounded-lg'>
          ...
          {/* 安全提示 */}
          {validationResult.securityNotes.length > 0 && (<div className= 'p-3 bg-blue-900/20 border border-blue-800/50 rounded-lg'>
              <div className= 'flex items-start space-x-2'>
          ...
          {/* 建议 */}
          {validationResult.suggestions.length > 0 && showDetails && (<div className= 'p-3 bg-green-900/20 border border-green-800/50 rounded-lg'>
              <div className= 'flex items-start space-x-2'>
      ...
      {/* 常用示例 */}
      {showSuggestions && !value.trim() && (<div className= 'p-3 bg-gray-700/30 rounded-lg'>
