...
import { AppError, ErrorDisplay } from './ErrorDisplay';
...
export const SecurityTestPanel = forwardRef<SecurityTestPanelRef, SecurityTestPanelProps>(
  ({ onTestStart, onTestProgress, onTestComplete, onTestError }, ref) => {
    // 状态管理
    ...
    const [detailedError, setDetailedError] = useState<AppError | null>(null);
    ...
    const handleUrlValidation = useCallback(
      (isValid: boolean, result?: URLValidationResult) => {
    ...
    const runTest = useCallback(async () => {
      ...
      try {
        const result = await securityEngine.runSecurityTest(config, progressData => {
    ...
    }, [config, isUrlValid, onTestError, onTestStart, onTestProgress, onTestComplete]);
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-2 gap-1 sm:gap-0">
                  ...
                  <div className="text-xs text-gray-300 font-medium bg-gray-600/50 px-2 py-0.5 rounded">
                    {key === 'quick' ? '1-2分钟' : key === 'standard' ? '3-5分钟' : '5-10分钟'}
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {moduleOptions.map(module => {
              ...
              return (
                <div
                  key={module.key}
                  className={`group relative p-3 rounded-lg border transition-all duration-300 overflow-hidden ${
                    isRunning ? 'cursor-not-allowed opacity-60' : 'cursor-pointer hover:shadow-md'
                  } ${
                    isEnabled
                ...
                >
                  ...
                  {isEnabled && (
                    <div
                      className={`absolute top-0 left-0 right-0 h-1 bg-current rounded-t-xl opacity-80`}
                    ></div>
                  )}
                  ...
                  <h4
                    className={`font-bold mb-2 text-sm transition-colors duration-200 ${
                      isEnabled
