...
位置：`backend/modules/middleware/logger.ts`
...
位置：`backend/modules/utils/logger.ts`
...
位置：`backend/modules/utils/ErrorMonitoringSystem.ts`
...
位置：`backend/modules/utils/ErrorLogAggregator.ts`
...
> 具体字段请参考 `backend/modules/system/routes/monitoring.ts`。

## 配置项

配置中心位置：`backend/modules/config/ConfigCenter.ts`
