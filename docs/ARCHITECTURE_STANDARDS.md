...
- 使用 `backend/modules/middleware/responseFormatter.ts` 导出的 `response`
  中间件。
...
│   ├── environments/         # 环境变量管理
│   ├── schedules/            # 定时运行
│   ├── testing/              # 测试相关（controller/service/route）
│   └── uat/                  # UAT 反馈（controller/service/route）
├── controllers/              # 传统控制器（逐步迁移）
├── services/                 # 传统服务（逐步迁移）
├── repositories/             # 数据访问层
└── routes/                   # 通用路由入口（非模块化）
```
...
- 新功能优先放入 `backend/modules/<domain>/`。
...
> 说明：历史示例中的 `backend/src/api/...` 结构为早期参考，当前以
> `backend/modules/` 为主。
