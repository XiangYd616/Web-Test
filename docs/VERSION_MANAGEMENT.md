...
本项目采用统一的版本管理策略,通过 `versions.json` 集中管理所有依赖版本,确保多个 package 之间的依赖版本一致性。
...
2. **定期检查**: 每次 PR 前运行 `npm run version:check`
...
- **每周**: 运行 `npm run version:check` 检查冲突
- **每月**: 审查依赖更新,更新非破坏性版本
- **每季度**: 评估主版本升级,制定迁移计划
- **每次发布前**: 生成版本报告并归档
