...
CREATE INDEX IF NOT EXISTS idx_collections_default_environment_id ON collections(default_environment_id);
CREATE INDEX IF NOT EXISTS idx_collections_created_by ON collections(created_by);
CREATE INDEX IF NOT EXISTS idx_collections_created_at ON collections(created_at DESC);

-- 兼容已存在的 collections 表，补齐缺失字段/约束
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE tc.table_name = 'collections'
          ...
          AND tc.constraint_name = 'collections_default_environment_id_fkey'
    ) THEN
        ALTER TABLE collections
            ADD CONSTRAINT collections_default_environment_id_fkey
...
CREATE INDEX IF NOT EXISTS idx_performance_test_results_test_result_id ON performance_test_results(test_result_id);
...
ALTER TABLE IF EXISTS security_test_results
...
CREATE INDEX IF NOT EXISTS idx_api_test_results_test_result_id ON api_test_results(test_result_id);

ALTER TABLE IF EXISTS api_test_results
...
CREATE TABLE IF NOT EXISTS performance_baselines (
    benchmark_id VARCHAR(100) PRIMARY KEY REFERENCES performance_benchmarks(id) ON DELETE CASCADE,
    baseline_data JSONB NOT NULL DEFAULT '{}',
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX IF NOT EXISTS idx_performance_baselines_benchmark ON performance_baselines(benchmark_id);

-- 性能基准测试结果表
CREATE TABLE IF NOT EXISTS performance_benchmark_results (
    id VARCHAR(100) PRIMARY KEY,
    benchmark_id VARCHAR(100) NOT NULL REFERENCES performance_benchmarks(id) ON DELETE CASCADE,
    status VARCHAR(20) NOT NULL,
    environment VARCHAR(50) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_performance_benchmark_results_benchmark ON performance_benchmark_results(benchmark_id);
CREATE INDEX IF NOT EXISTS idx_performance_benchmark_results_executed_at ON performance_benchmark_results(executed_at DESC);
...
-- 兼容已存在的 uploaded_files 表，补齐缺失字段/约束
...
CREATE TABLE IF NOT EXISTS test_templates (
    ...
    description TEXT,
    engine_type VARCHAR(20) NOT NULL CHECK (engine_type IN ('api', 'compatibility', 'infrastructure', 'security', 'seo', 'stress', 'ux', 'website', 'performance', 'accessibility')),
...
-- 兼容已存在的 engine_status 表，重建 engine_type 检查约束
DO $$
BEGIN
    IF to_regclass('public.engine_status') IS NOT NULL THEN
        ALTER TABLE engine_status
            DROP CONSTRAINT IF EXISTS engine_status_engine_type_check;
        ALTER TABLE engine_status
            ADD CONSTRAINT engine_status_engine_type_check
            CHECK (engine_type IN ('api', 'compatibility', 'infrastructure', 'security', 'seo', 'stress', 'ux', 'website', 'performance', 'accessibility'));
    END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS monitoring_sites (
    ...
    name VARCHAR(255) NOT NULL,
    url TEXT NOT NULL,
    monitoring_type VARCHAR(50) NOT NULL DEFAULT 'uptime',
...
CREATE INDEX IF NOT EXISTS idx_uploaded_files_owner ON uploaded_files(owner_type, owner_id);
...
('email_verification_required', '{"value": false}', '注册邮箱验证开关', 'features', true),
