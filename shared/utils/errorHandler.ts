 ...
 *
 * 此文件整合了所有重复的错误处理逻辑：
 * - backend/modules/middleware/errorHandler.js
 * - backend/modules/api/middleware/errorHandler.js
 * - backend/modules/utils/errorHandler.js
...
export class ErrorHandler {
  ...
  private logError(error: StandardError): void {
    ...
    const configIdx = severityOrder.indexOf(logLevel as ErrorSeverity);
    const errorIdx = severityOrder.indexOf(error.severity);
    if (configIdx > errorIdx) return;
