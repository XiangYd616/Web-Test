...
jest.mock('../../../backend/modules/middleware/auth', () => ({
...
jest.mock('../../../backend/modules/middleware/logger', () => ({
...
jest.mock('../../../backend/modules/database/pgModels', () => ({
  models: {
...
jest.mock('../../../backend/modules/collections/services/CollectionManager', () => {
  return jest.fn().mockImplementation(() => mockManager);
});

jest.mock('../../../backend/modules/auth/services/sessionManager', () => {
  return jest.fn().mockImplementation(() => ({}));
});

jest.mock(
  '../../../backend/modules/auth/routes/mfa',
  () => {
    const express = require('express');
...
const { response } = require('../../../backend/modules/middleware/responseFormatter');
const collectionsRouter =
  require('../../../backend/modules/collections/routes/collections').default;
