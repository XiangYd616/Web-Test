const UXTestEngine = require('../../../backend/modules/engines/ux/UXTestEngine').default;

jest.mock('../../../backend/modules/config/database', () => ({
  query: jest.fn().mockResolvedValue({ rows: [] }),
}));

jest.mock('../../../backend/modules/testing/repositories/testRepository', () => ({
  ...
  test('应在未确认Puppeteer时抛出错误', async () => {
    ...
    await expect(
      engine.executeTest({
        ...
        confirmPuppeteer: false,
      })
    ).rejects.toThrow('配置验证失败: "confirmPuppeteer" must be [true]');
  });
