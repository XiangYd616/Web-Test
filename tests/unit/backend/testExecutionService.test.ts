...
describe('TestExecutionService', () => {
  ...
  beforeAll(async () => {
    try {
      const modulePath = '../../../backend/modules/testing/services/TestExecutionService';
      ...
      TestExecutionService = class {
        public activeTests: Map<string, unknown>;
        public testResults: Map<string, unknown>;
        ...
        calculatePerformanceScore(loadTime: number, _metrics: Record<string, any>): number {
  ...
  afterEach(() => {
    // 清理测试数据
    if ((testService as any).activeTests) {
      (testService as any).activeTests.clear();
    }
    if ((testService as any).testResults) {
      (testService as any).testResults.clear();
    }
  });
