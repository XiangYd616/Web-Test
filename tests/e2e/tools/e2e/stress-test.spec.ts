...
class StressTestPage {
  ...
  async startTest() {
    ...
    // 等待测试开始
    await expect(this.page.locator('[data-testid="test-status"]')).toHaveText('运行中', {
      timeout: 10000,
    });
  }
  ...
  async waitForTestCompletion(timeout: number = 60000) {
    await expect(this.page.locator('[data-testid="test-status"]')).toHaveText('已完成', {
      timeout,
    });
  }
  ...
  async getProgressInfo() {
    const progress = await this.page.getAttribute(
...
test.describe('压力测试完整流程', () => {
  ...
  test.describe('测试配置', () => {
    ...
    test('高级配置设置', async ({ page }) => {
      await stressTestPage.fillBasicConfig(
        stressTestConfig.url,
        stressTestConfig.concurrency,
        stressTestConfig.duration
      );
      await stressTestPage.fillAdvancedConfig(
        stressTestConfig.rampUp,
        stressTestConfig.rampDown,
  ...
  test.describe('测试执行', () => {
    ...
    test('渐进加压测试验证', async ({ page }) => {
      ...
      expect(progress.phase).toMatch(/加压|启动/);

      // 等待进入主测试阶段
      await page.waitForTimeout(8000);
      progress = await stressTestPage.getProgressInfo();
      expect(progress.phase).toMatch(/测试|稳定/);
  ...
  test.describe('结果导出', () => {
    test('JSON格式导出', async ({ page: _page }) => {
      await stressTestPage.fillBasicConfig(
        stressTestConfig.url,
        stressTestConfig.concurrency,
        stressTestConfig.duration
  ...
  test.describe('错误处理', () => {
    test('网络连接失败处理', async ({ page: _page }) => {
      // 使用无法访问的URL
      ...
      // 验证错误处理
      const errorMessage = await stressTestPage.getErrorMessage();
      expect(errorMessage).toMatch(/连接失败|网络错误/);
    });

    test('超时处理', async ({ page: _page }) => {
      // 使用延迟很长的URL和短超时
    ...
    test('服务器错误响应处理', async ({ page: _page }) => {
      // 使用返回500错误的URL
